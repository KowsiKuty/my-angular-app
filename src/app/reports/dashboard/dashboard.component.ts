...
searchData(data, type: number) {
  if(!data.bpa_no) {
    this.notificationService.showError("Please Select BPA No!");
    return;
  }
  if(type == 1) {
    this.SpinnerService.show();
  this.reportService.getReportPO(data.bpa_no, data.pca_no, type).subscribe(response => {
      this.SpinnerService.hide();
  this.showDashboard = true;
      this.bpaData = response.BPAs[0];
...
this.shuffleColors();
    ...
    if(type === 2) {
      ...
      this.reportService.getReportPOdownld(data.bpa_no, type).subscribe((response) => {
...
// }
          ...
          .subscribe(x => {
            ...
            if (atBottom) {
              if (this.has_deptnext === true) {
                this.reportService.getDepartmentScrolls(this.deptInput.nativeElement.value, this.deptcurrentpage + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
  ...
  public displayFnDeptName(autoDept?: DepartmentName): string | undefined {
      return autoDept ? autoDept.no : undefined;
    }
  
    get autoDept() {
      return this.searchBPA.get('bpa_no');
    }
    departmentName(primaykey1) {
    this.reportService.getDepartmentFilters(primaykey1)
      .subscribe((results) => {
        let datas = results["data"];
  ...
  inputdepartmentName() {
    this.reportService.getDepartmentFilters(this.deptInput.nativeElement.value)
      .subscribe((results) => {
        let datas = results["data"];
        this.deptNameList = datas;
        let datapagination = results["pagination"];
  ...
  if(type == 1) {
    this.SpinnerService.show();
  this.reportService.getReportPO(bpaNo?.no, '', type).subscribe(response => {
      this.SpinnerService.hide();
  this.showDashboard = true;
      this.bpaData = response.BPAs[0];
      this.totalUtilized = this.bpaData?.par_utilized_amount || 0;
...
this.shuffleColors();
    ...
    if(type === 2) {
      ...
      this.reportService.getReportPOdownld(this.payloads, type).subscribe((response) => {
         if (response instanceof Blob && response.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
    const blob = new Blob([response], { type: response.type });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    let date = new Date()
    a.download =  "Budget_report_" + date + ".xlsx";
    a.click();
    window.URL.revokeObjectURL(url);
         }
      }, (error: HttpErrorResponse) => {
