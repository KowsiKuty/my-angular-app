...
import { RemsService } from '../rems/rems.service';
import { FormBuilder, FormGroup, Validators, FormControl, FormArray } from '@angular/forms';
import { NativeDateAdapter, DateAdapter, MAT_DATE_FORMATS } from '@angular/material/core';
import { formatDate, DatePipe } from '@angular/common';
import { finalize, switchMap, tap, distinctUntilChanged, debounceTime, catchError } from 'rxjs/operators';
...
import { Rems2Service } from '../rems/rems2.service';
...
export interface posubmoduleistss {
  id: string;
  name: string;
}
...
export class ReportsComponent implements OnInit {
  ...
  @ViewChild("posubmodule") matposubmoduleAutocomplete: MatAutocomplete;
  @ViewChild("poInput") poInput: any;

    posubmodulelist: Array<posubmoduleistss>;
  isBudgetReport:boolean = false
  pcareposrtdash_b:boolean = false
isPoDetailsReport:boolean = false
isAssetDetailsReport:boolean = false
  ...
  glsummaryform: FormGroup;
  glstatementreportform: FormGroup;
  ...
  selectedFile:any
  filearray: FormData = new FormData();
 ...
 @ViewChild('glnoInput') glnoInput: ElementRef;
  gl_no:any[]=[]
  summarylist:any[]=[]
  reportsummarylist:any
  fileuploadsummarylist:any
   prev_glsummary:boolean=false;
  next_glsummary:boolean=true;
  glsummary_page:number=1;
   prev_reportsummary:boolean=false;
  next_reportsummary:boolean=true;
  reportsummary_page:number=1;
  ...
  sec_isAssetDetailsReport: boolean=false
  ...
  ngOnInit(): void {
    ...
    this.getPOList()
     ...
     this.glsummaryform = this.fb.group({
     from_date:[''],
     to_date:[''],
     branch_id:[''],
     gl:[''],
    })
    this.glstatementreportform = this.fb.group({
     from_date:[''],
     to_date:[''],
     branch_id:[''],
     gl:[''],
...
// }
  ...
  this.POModulereport();
  this.POReportExcel.get('par_no').setValue(this.parNo);
  this.PoUtilSearch();
}
    ...
    this.regReportSearch.get('state_reg').valueChanges
      ...
      let bpa_no: String = "";
       ...
       this.getparnoFK();
    this.POReportExcel.get('par_no').valueChanges
      .pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
        ...
        switchMap(value => this.reportService.getparnoFK(value)
          .pipe(
            finalize(() => {
              this.isLoading = false
            }),
      ...
      this.getmepFK();
    this.POReportExcel.get('mep_no').valueChanges
      .pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
        ...
        switchMap(value => this.reportService.getmepFK(value)
          .pipe(
            finalize(() => {
              this.isLoading = false
            }),
...
CBS_tb_screen:boolean=false
  ...
  this.isMasterReport =false;
    ...
    this.isAssetDetailsReport=false
    this.sec_isAssetDetailsReport=false
    this.isPoDetailsReport=false
    this.isBudgetReport=false
    this.pcareposrtdash_b=false
...
POModulereportt(data){
  ...
  if(data.id==1){
    ...
    // setTimeout(() => {
    this.isAssetDetailsReport=true
    ...
    this.sec_isAssetDetailsReport=true
   this.isAssetDetailsReport=false
    this.sharedService.ReportsPO.next(5)
...
resetAllFlags() {
  const flags = [
    'isRCNReportsummarypage', 'isRCN', 'isProvisionReport', 'isLeaseReport', 'isBeforeRunReport',
    'isAfterRunReport', 'isPremiseReport', 'isMasterReport', 'isPaymentReport', 'isMemoType', 'isType',
    'isOccupancy', 'isRent', 'isLandlord', 'isAgreement', 'ispremiseown', 'occupancy', 'rent',
    'agreement', 'ispremiselea', 'isoccuown', 'isocculea', 'islandown', 'islandlea', 'isrentlea',
    'isrentown', 'isClosedOccpancy', 'isOccpancyUsageType', 'isHoldRent', 'isOccpancyUsageCode',
    'isUnregLandlord', 'isNRILandlord', 'isExpiredAgree', 'isBuilding', 'isExpiredRentSchedule',
    'ispremiseactive', 'ispremiseterminated', 'MemoShow', 'isPOReport', 'isBudgetReport',
    'isPoDetailsReport', 'isAssetDetailsReport', 'inwardReportTab'
  ];
...
getPOList() {
  this.SpinnerService.show();
  this.reportService.getPOListNew()
    .subscribe(results => {
      this.SpinnerService.hide();
      let data = results["data"];
      console.log("data for PO list=======================> ", data);
      this.PoDataArray = data;
      if(this.parNo){
   const targetModule = this.PoDataArray.find(x => x.text === "PCA Utilization Report");
      if (targetModule) {
        this.subModuleName = targetModule;
        // this.POModulereportt(targetModule); // Uncomment if you want to trigger report
      }
      }
      // Move this inside after PoDataArray is updated
 ...
 getposubmod() {
    ...
    this.poform.get("pomodule")
      .valueChanges.pipe(
        ...
        switchMap((value) =>
          this.reportService
            .getposubmodFKdd(this.poform.value.mepno, value, 1)
            .pipe(
              finalize(() => {
                this.isLoading = false;
              }),
      ...
      .subscribe((results: any[]) => {
        let datas = results["data"];
        this.posubmodulelist = datas;
      });
  }

   getposubmodFK() {
      if (this.poform.value.prdetails.length > 0) {
      ...
      this.SpinnerService.show();
      let productData = this.poform.value.mepno;
      this.reportService.getposubmodFK(productData, "").subscribe(
        (results: any[]) => {
    ...
    autocompleteposubmodScroll() {
      ...
      setTimeout(() => {
        if (
          this.matposubmoduleAutocomplete &&
          this.autocompleteTrigger &&
          this.matposubmoduleAutocomplete.panel
        ) {
          fromEvent(this.matposubmoduleAutocomplete.panel.nativeElement, "scroll")
            .pipe(
              map(
                (x) => this.matposubmoduleAutocomplete.panel.nativeElement.scrollTop
              ),
              takeUntil(this.autocompleteTrigger.panelClosingActions)
            )
            .subscribe((x) => {
              const scrollTop =
                this.matposubmoduleAutocomplete.panel.nativeElement.scrollTop;
              const scrollHeight =
                this.matposubmoduleAutocomplete.panel.nativeElement.scrollHeight;
              const elementHeight =
                this.matposubmoduleAutocomplete.panel.nativeElement.clientHeight;
              const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
              if (atBottom) {
                  ...
                  this.reportService
                    ...
                    .subscribe(
                      (results: any[]) => {
                        let datas = results["data"];
                        let datapagination = results["pagination"];
                        this.posubmodulelist = this.posubmodulelist.concat(datas);
                        if (this.posubmodulelist.length >= 0) {
     ...
     public displayFnposubmodule(commodity?: posubmoduleistss): string | undefined {
      ...
      craetewisefintb() {
        ...
        let jv = this.uploadjv;
        let jw = this.uploadjw;
        let fa = this.uploadfa;
        let expense = this.uploadcbcc;
      ...
      prepareWisefintb(){
        let data ={date: this.datePipe.transform(
          this.cbsWisefinTBDate.value,
          "yyyy-MM-dd",
        )}
        this.SpinnerService.show();
        this.reportService
          .prepareWisefintb(data)
          .subscribe((results) => {
            this.SpinnerService.hide();
            if (results['code'] != undefined) {
              this.notification.showError(results.code);
        ...
        tbsummary() {
          if(this.cbsWisefinTBDate.value == undefined || this.cbsWisefinTBDate.value == null || 
            this.cbsWisefinTBDate.value ==''){
              this.notification.showError('Please Choose the Date.')
              return false
            }
            this.prepareEnable = false
      ...
      return config
        ...
        wisefinTBsummary:any
        wisefinTBreset(){
          this.getWisefinTBsummary()
        }
has_nextTBsummary = false        
has_previousTBsummary = false   
presentpageTBsummary =1  
isTBsummary =false  
getWisefinTBsummary(){
  this.reportService.wisefin_tb_summary().subscribe((results: any[]) => {
      let datas = results["data"];
      this.wisefinTBsummary = results["data"];
      let dataPagination = results['pagination'];
      console.log("datasupload", datas)
      if (datas.length >= 0) {
        this.has_nextTBsummary = dataPagination.has_next;
        this.has_previousTBsummary = dataPagination.has_previous;
        this.presentpageTBsummary = dataPagination.index;
        this.isTBsummary = true;
      }
      if (datas.length <= 0) {
        this.isTBsummary = false;
  ...
  clicktodowntemp(type,data){
    this.reportService.download_TB_template(type).subscribe((res=>{
      let binaryData = [];
      binaryData.push(res);
      let downloadUrl = window.URL.createObjectURL(new Blob(binaryData));
      let link = document.createElement("a");
  ...
  onFileSelected(event: any): void {
    ...
    if (file && (file.type === 'application/vnd.ms-excel' || file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')) {
      ...
      this.filearray.append('file', file, file.name);
    } else {
  ...
  uploadFile() {
    ...
    this.reportService.glfileupload(this.filearray).subscribe(response => {
  ...
  reset_file(){
     this.fileuploadform.reset()
        this.selectedFile=''
        this.filearray=  new FormData();
  }
   ppremployee_branchdrop(){
    this.reportService.getempbranchedrop('',1).subscribe(data=>{
      ...
      if (
        ...
        fromEvent(this.ppr_matempbranchname.panel.nativeElement, 'scroll')
          ...
          .subscribe(x => {
            ...
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.pprhas_empbranchnxt === true) {
                this.reportService.getempbranchedrop(this.glsummaryform.get('branch_id').value,this.pprhas_empbranchpage+1)
                  .subscribe((data: any[]) => {
                    let datas = data["data"];
                    let datapagination = data["pagination"];
        ...
        x=>{
          ...
          if(atBottom){
           if(this.has_glnonext){
            this.reportService.gl_noList(this.glno_presentpage+1,this.glsummaryform.get('gl').value).subscribe((data:any)=>{
...
getglnum(){
  this.reportService.gl_noList(1,'').subscribe(data =>{
...
getglsummary(){
  ...
  let x=this.glsummaryform.value
  if(x.gl===''||x.gl===null||x.gl===undefined){
this.notification.showError("choose GL")
return
  }
   if(x.from_date===''||x.from_date===null||x.from_date===undefined){
this.notification.showError("choose From Date")
return
  }
   if(x.to_date===''||x.to_date===null||x.to_date===undefined){
this.notification.showError("choose To Date")
return
  }
  let payload={
    "gl":x.gl?.no,
    "branch_id":x.branch_id?.id ? x.branch_id.id :'',
    "from_date": this.datePipe.transform(x.from_date, 'yyyy-MM-dd'),
    "to_date":this.datePipe.transform(x.to_date, 'yyyy-MM-dd'),
    "type":"summary"}
    console.log(payload)
    this.SpinnerService.show()
    this.reportService.getglsummaryapi(payload,this.glsummary_page).subscribe(res=>{
      this.SpinnerService.hide()
      this.summarylist=res['data']
        let pagination=res['pagination']
        if(this.summarylist.length>0){
          this.prev_glsummary=pagination.has_previous;
          this.next_glsummary=pagination.has_next;
          this.glsummary_page=pagination.index; 
        }  
    })
}
 branchgl_previous(){
    this.glsummary_page=this.glsummary_page-1
      this.getglsummary()
...
public getemployeebranchinterface(data?:empbranch):string|undefined{
  ...
  public glnointreface(data?:glno):string | undefined{
    ...
    preparegl(){
      let x=this.glstatementreportform.value
  if(x.gl===''||x.gl===null||x.gl===undefined){
this.notification.showError("choose GL")
return
  }
...
return
  ...
  let payload={
    ...
    //      this.SpinnerService.hide()
    //     }, 5000);
    this.reportService.getglsummaryapi(payload,this.reportsummary_page).subscribe(res=>{
    ...
    refreshgl(){
      this.SpinnerService.show()
      this.reportService.gl_statement_ref_api().subscribe(res=>{
        this.SpinnerService.hide()
        this.reportsummarylist=res.GL_STATEMENT
      })
    }
    report__Download(){
      this.btn_spinner_fadatadow=true;
    this.fadatadown=true;
    this.reportService.gl_file_download().subscribe((res: any) => {  
        ...
        if (res?.type=='application/json') {
          this.toastr.warning("UNEXPECTED_ERROR");
            ...
            a.href = url;
            a.download = `GL_Statement_Report _${formattedDate}.xlsx`;
    ...
    reset_gl_tab(){
      this.glstatementreportform.reset()
      this.glsummaryform.reset()
      this.reset_file()
    ...
    refreshfileupload(){
       this.SpinnerService.show()
      this.reportService.fileupload_ref_api().subscribe(res=>{
        this.SpinnerService.hide()
        this.fileuploadsummarylist=res.narration
      })
    }
    reset_summary(){
      this.glsummaryform.reset()
    ...
    reset_statement_summary(){
       this.glstatementreportform.reset()
      // this.reportsummarylist=''
    }
    getglnuminput(){
      
  this.reportService.gl_noList(1,this.glsummaryform.get('gl').value).subscribe(data =>{
    this.gl_no =data['data'];
      let pagination=data['pagination']
               if(this.gl_no.length>0){
                 this.has_glnonext=pagination.has_next;
  ...
  ppremployee_branchdropinput(){
    this.reportService.getempbranchedrop(this.glsummaryform.get('branch_id').value,1).subscribe(data=>{
      this.pprempbranchnamelist=data['data']
      let datapagination = data["pagination"];
      if (this.pprempbranchnamelist.length > 0) {
        this.pprhas_empbranchnxt = datapagination.has_next;
   ...
   getglnuminput_statement(){
      
  this.reportService.gl_noList(1,this.glstatementreportform.get('gl').value).subscribe(data =>{
    this.gl_no =data['data'];
      let pagination=data['pagination']
               if(this.gl_no.length>0){
                 this.has_glnonext=pagination.has_next;
  ...
  ppremployee_branchdropinput_statement(){
    this.reportService.getempbranchedrop(this.glstatementreportform.get('branch_id').value,1).subscribe(data=>{
      this.pprempbranchnamelist=data['data']
      let datapagination = data["pagination"];
      if (this.pprempbranchnamelist.length > 0) {
        this.pprhas_empbranchnxt = datapagination.has_next;
