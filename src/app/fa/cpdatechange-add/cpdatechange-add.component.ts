...
import { finalize, map, reduce, switchMap, takeUntil } from 'rxjs/operators';
...
export class CpdatechangeAddComponent implements OnInit {
  ...
  getassetcategorysummary(pageNumber = 1, pageSize = 10) {
    ...
    if(this.cpdatechangeadd.get('assetid').value !=null && this.cpdatechangeadd.get('assetid').value !=""){
     ...
     if(this.cpdatechangeadd.get('crno').value !=null && this.cpdatechangeadd.get('crno').value !=""){
      searh=searh+"&crno="+this.cpdatechangeadd.get('crno').value;
     }
    data['page']=searh;
    this.spinner.show();
    this.Faservice.getcpdateaddsummary(data)
      .subscribe((result:any) => {
        ...
        for(let i=0;i<this.assetcatlist.length;i++){
          this.assetcatlist[i]['is_checked']=false;
          if(this.assetcatlist[i]['validations'] ==Object.create({})){
...
resets(){
  ...
  this.cpdatechangeadd.get('category').patchValue('');
  this.cpdatechangeadd.get('branch').patchValue('');
  this.cpdatechangeadd.get('crno').patchValue('');
  this.getassetcategorysummary();
}
searchdata(){
  this.presentpagecpadd=1;
  ...
  if(this.cpdatechangeadd.get('assetid').value !=null && this.cpdatechangeadd.get('assetid').value !=""){
   ...
   console.log(searh);
   this.spinner.show();
    this.Faservice.getcpdateaddsummary(data).subscribe((data:any)=>{
      console.log(data);
      this.spinner.hide();
      if (data.code!=undefined && data.code!="" && data.code!=undefined){
