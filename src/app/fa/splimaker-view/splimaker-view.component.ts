import { Component, OnInit, Inject, Output, EventEmitter, AfterViewChecked, HostListener } from '@angular/core';
...
btndisablepopup:boolean=false;
  ...
  ngOnInit(): void {
    ...
    this.Assettotal = data.assetcost;
  ...
  SubmitAssetSplit() {
    ...
    if(Math.trunc(this.Assettotal) !=Math.trunc(this.sum1)){
    ...
    //   }
    // }
    if (Math.trunc(this.Assettotal) < Math.trunc(this.sum) || Math.trunc(this.Assettotal) > Math.trunc(this.sum)) {
  ...
  ValueChangeAssetTotal(data, index) {
    ...
    // })
    this.sum1=0;
    console.log((this.myForm.get('split_detail') as FormArray).value);
    for(let i=0;i<(this.myForm.get('split_detail') as FormArray).length;i++){
      console.log(Number(((this.myForm.get('split_detail') as FormArray).at(i) as FormGroup).get('value').value));
      console.log(this.sum1);
      this.sum1=this.sum1 + (((this.myForm.get('split_detail') as FormArray).at(i) as FormGroup).get('value').value);
    }
    console.log(this.sum1);
    if(Math.trunc(this.Assettotal)<Math.trunc(this.sum1)){
