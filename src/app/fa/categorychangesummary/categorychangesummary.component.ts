...
export class CategorychangesummaryComponent implements OnInit {
  ...
  searchData: any;
  constructor(private Faservice: faservice,private toastr:ToastrService,private datePipe:DatePipe,private router: Router,private formBuilder: FormBuilder,private faService: Fa3Service,private spinner:NgxSpinnerService) { }
...
getCategoryChange(pageNumber=1, pageSize=5) {
  if(this.categorychangesearch){
    ...
    if(categorychangechangesearch.crno !=null && categorychangechangesearch.crno !=''){
      this.searchData.crno = categorychangechangesearch.crno;
...
// search 
  ...
  Assetbarcode(){
    let keyvalue: String = "";
        ...
        clearSearch(){
          ...
          this.categorychangesearch.controls['fromdate'].reset('')
          this.categorychangesearch.controls['todate'].reset('')
        ...
        fa_min_Expense_Reports_Download(){
          if ((this.categorychangesearch.get('fromdate').value == '' || this.categorychangesearch.get('fromdate').value == null || this.categorychangesearch.get('fromdate').value == undefined) || (this.categorychangesearch.get('todate').value == '' || this.categorychangesearch.get('todate').value == null || this.categorychangesearch.get('todate').value == undefined)) {
            this.toastr.warning("Please Select from and todate");
            return false;
          }

          let fromdate = this.datePipe.transform((this.categorychangesearch.get('fromdate').value), 'yyyy-MM-dd');
          let todate = this.datePipe.transform(this.categorychangesearch.get('todate').value, 'yyyy-MM-dd');
          let payload={
            "from_date":fromdate,
            "to_date":todate
          }
          this.spinner.show();
          this.Faservice.fa_category_change_downloads(payload).subscribe(result=>{
            this.spinner.hide();
            if(result.type=='application/json'){
              ...
              link.download = 'FA Cat Change Reports_'+ date +".xlsx";
              link.click();
  ...
  expandentrydata(index){
    this.categoryadd[index]['sub_enb']=!this.categoryadd[index]['sub_enb'];
   }
