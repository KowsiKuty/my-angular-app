...
selectPage:number=10;
  constructor(private toastr:ToastrService,private router: Router,private notification: NotificationService,private faService:Fa3Service,private formBuilder:FormBuilder,private datePipe:DatePipe,private spinner:NgxSpinnerService) { }
  ...
  ngOnInit(): void {
    ...
    this.assetsalesapprove(1,this.selectPage);
  ...
  assetsalesapprove(pageNumber=1, pageSize=5){
    ...
    // if(this.salesSummaryApprove){
      ...
      if(this.salesSummaryApprove.get('crno').value != null && this.salesSummaryApprove.get('crno').value != '' && this.salesSummaryApprove.get('crno').value != undefined){
        this.searchData.crno = valuechangeadd.crno;
      }
...
clearSearch(){
  ...
  this.assetsalesapprove(this.presentpage,this.selectPage);
...
Assetbarcode(){
  let keyvalue: String = "";
    ...
    nextClick() {
      if (this.has_next === true) {
        ...
        this.assetsalesapprove(this.presentpage + 1, this.selectPage)
      }
  ...
  if (dataApprove == false) {
    ...
    rejectSummary(){
      ...
      let data_min:any=(((this.approve.length*20000)/1000)/60).toFixed(2);
      this.toastr.warning('Wait for '+(data_min).toString() +' minutes','',{timeOut:(this.approve.length*5000),progressBar:true,progressAnimation:'decreasing'});
      this.spinner.show();
      this.faService.assetsaleapprove(this.approve, this.reasons).subscribe(res =>{
