...
export class TransfermakeraddComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService,private notification: NotificationService,
  ...
  fetch_assettransfer(pageNumber=1, pageSize=this.selectPage) {
    this.newvalue=[];
    this.searchData={};
    if(this.transferaddsearch){
      let transfermakeradd=this.transferaddsearch.value
      if((transfermakeradd.capdate_Value != null && transfermakeradd.capdate_Value != '' && transfermakeradd.capdate_Value != undefined)  ){
        var tranferdate=this.datePipe.transform(transfermakeradd.capdate_Value, 'yyyy-MM-dd')
        this.searchData.capdate=tranferdate
      }
      if(transfermakeradd.asset_value !=null && transfermakeradd.asset_value !='' && transfermakeradd.asset_value !=undefined ){
          this.searchData.assetdetails_value = transfermakeradd.asset_value;
  
      }
      if(transfermakeradd.category !=null && transfermakeradd.category !='' && transfermakeradd.category !=undefined && transfermakeradd.category.id !=undefined){
        this.searchData.assetcat_id = transfermakeradd?.category?.id;

    }

    if(transfermakeradd.barcode !=null && transfermakeradd.barcode !='' && transfermakeradd.barcode !=undefined && transfermakeradd.barcode !=undefined){
      this.searchData.barcode = transfermakeradd?.barcode;

  }

  if(transfermakeradd.branch_name !=null && transfermakeradd.branch_name !='' && transfermakeradd.branch_name !=undefined && transfermakeradd.branch_name.id !=undefined){
    this.searchData.branch_id = transfermakeradd?.branch_name?.id;

}
if(transfermakeradd.crno !=null && transfermakeradd.crno !='' && transfermakeradd.crno !=undefined && transfermakeradd.crno !=undefined){
  this.searchData.crno = transfermakeradd?.crno;
...
nextClick() {
  if (this.has_next === true) {
      ...
      this.fetch_assettransfer(this.presentpage + 1, this.selectPage);
    }
     ...
     onCheckboxChange(event,value,i:any){
      ...
      if (event.currentTarget.checked) {
        ...
        this.newvalue.push({
          ...
          "capdate":this.datePipe.transform(value['capdate'],"yyyy-MM-dd")
          
        }); 
        this.fetchAssetTransfer[i]['checkbox']=true;
        this.transfer_date.push({
          "date":value['capdate']
        })
      } 
      else {
        this.fetchAssetTransfer[i]['checkbox']=false;
...
// end location 
 selectall(event){
  if (event.currentTarget.checked) {
    ...
    for(let i=0;i<this.fetchAssetTransfer.length;i++){
      let value:any=this.fetchAssetTransfer[i];
      this.fetchAssetTransfer[i]['checkbox']=true;
      this.branchshort.push({
        "branchid":value['branch_id']})
      this.newvalue.push({
...
submit(){
  ...
  for(let i=0;i<this.newvalue.length;i++){
    if((this.newvalue[i]['capdate']< dateformat) == false ){
  ...
  let data_min:any=(((this.transfer.length*20000)/1000)/60).toFixed(2);
  this.toastr.warning('Wait for '+data_min.toString()  +' minutes','',{timeOut:(this.transfer.length*20000),progressBar:true,progressAnimation:'decreasing'});
  this.spinner.show();
  this.faService.assettranfer(this.transfer, this.reasons,dateformat).subscribe(res =>{
...
focuslocation($event){  
  if (this.locationInput.nativeElement.value === '' && this.branchid === undefined) {
    ...
    clearSearch(){
      ...
      this.fetch_assettransfer(1, 10);
      
    }
    paginationChange(event:any){
      console.log(event);
      console.log(this.selectPage);
      this.fetch_assettransfer(1, this.selectPage);
    }
}
