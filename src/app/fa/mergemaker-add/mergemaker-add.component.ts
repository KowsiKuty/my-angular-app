...
btndisablepopup:boolean=false;
  ...
  PopUpdataToShow() {
    ...
    let assetvalue=this.done.map(v=>v.assetvalue);
    let assetvalue_path=assetvalue.reduce((index,value)=>{
      return index+Number(value);
    },0);
    console.log("Asset after merged value",assetvalue_path);
    this.MergeCreateForm.patchValue({
      value:assetvalue_path
    })
  ...
  MergeSubmit() {
    ...
    if(this.SelectedMainAsset==undefined || this .SelectedMainAsset==null || this.SelectedMainAsset==""){
      this.notification.showWarning("Please Selected Main Asset");
      return false;
    }
