...
detailsdata:any={};
  ...
  constructor(private notification:NotificationService,private datepipe:DatePipe,private Toast:ToastrService,private router:Router,private shareservice:faShareService,private fb:FormBuilder,private Faservice:faservice,private spinner:NgxSpinnerService) { }
  ...
  ngOnInit(): void {
    ...
    this.reportform=this.fb.group({
      'checker_doctype':new FormControl('')
    })
  ...
  checkersubmit_data(){
    console.log(this.Grb_by);
    if(this.Grb_by=='Y' || this.Grb_by=='N' || this.Grb_by=="Y" || this.Grb_by=="N"){
  ...
  getexpencecheckersummary(page=1,pageSize=10){
    ...
    dear['page']=search;
    this.spinner.show();
    this.Faservice.getExpensecheckersummary(dear,pageSize).subscribe((event: HttpEvent<any>) => {
 ...
 h: any[] = [];
  ...
  assetcheckerView(obj: any) {
    ...
    if(this.Grb_by=='Y'){
      ...
      this.shareservice.FACheckerDataView.next(obj);
    }
    else{
  ...
  resets(){
    ...
    if(this.Grb_by=="Y" || this.Grb_by=='Y' || this.Grb_by=="N" || this.Grb_by=='N'){
      this.getassetcategorysummary();
    }
    else{
      this.getexpencecheckersummary();
  ...
  Accountdetails(i){
    ...
    //    this.matdialog.open(this.opendialogdata,dialogConfig);
      console.log(i)
      this.spinner.show();
      this.Faservice.accounting_ddl(i.assetdetails_id.split('/')[0],this.has_accpage,'Expense','','').subscribe((result) => {
      if(result){
        this.spinner.hide();
        this.accountDetailslist = result['data'];
  ...
  expense_approve_reject(text_type:any){
    console.log('122');
    let approverList:Array<any>=[];
    for(let i=0;i<this.expdatalist.length;i++){
      if(this.expdatalist[i]['enb']==true){
        this.expdatalist[i]['type_exp']=text_type;
        this.expdatalist[i]['crnum']=this.expdatalist[i]['crnum'];
        this.expdatalist[i]['exp_code']=this.expdatalist[i]['exp_code'];
        this.expdatalist[i]['clearingdetails_id']=this.expdatalist[i]['clearingdetails_id'];
        approverList.push(this.expdatalist[i]);
      }
    }
    if(approverList.length==0){
      this.Toast.warning("Please Select The Data..");
      return false;
    }
    let approveconfirm=confirm("Are You Surely Want To "+text_type);
    if (approveconfirm==false){
      return false;
    }
    let data_post:any={"type":text_type,"data":approverList};
    this.spinner.show();
    this.Faservice.fa_expence_checker_approve(data_post).subscribe((result:any)=>{
      this.spinner.hide();
    ...
    (error:HttpErrorResponse)=>{
      this.spinner.hide();
      this.Toast.warning("Unable To Reach Server");
  ...
  checker_reportdowload(){
    if(this.reportform.get('checker_doctype').value=='' || this.reportform.get('checker_doctype').value==undefined || this.reportform.get('checker_doctype').value==null){
      this.notification.showWarning("Please Select the type");
      return false;
    }
    let params= '?doctype='+this.reportform.get('checker_doctype').value;
    this.spinner.show();
    this.Faservice.fa_checker_download(params).subscribe(result=>{
      this.spinner.hide();
          if(result.type=='application/json'){
            ...
            link.download = 'FA checker Reports_'+ date +".xlsx";
            link.click();
