...
export class CategorychangesummaryaddComponent implements OnInit {
  ...
  getcategorychangeAdd(pageNumber = 1, pageSize = 5) {
    ...
    if(this.categorychange){
      console.log(this.categorychange.value);
      ...
      if(valuechangeadd.crno !=null && valuechangeadd.crno !='' && valuechangeadd.crno !=undefined){
...
onCheckboxChange(event, value, index) {
  ...
  if (event.currentTarget.checked) {
    ...
    const categoryindex= this.categoryshort.findIndex(catid=>Number(catid.assetcatid) == Number(value.assetcat_id))
...
private change_category(keyvalue) {
  this.faService.getAssetSearchFilter(keyvalue, 1)
    .subscribe((results: any[]) => {
      ...
      //   let filterdata=this.changeCategoryValue.findIndex(catid=>Number(catid.id)===Number(value.assetcatid))
