...
export class TransfermakersummaryComponent implements OnInit {
  ...
  constructor(private faService: Fa3Service, private formBuilder: FormBuilder,private notification:NotificationService,
  ...
  getAssetTransfer(pageNumber = 1, pageSize = 5) {
    if(this.transfermarkersearch){
      ...
      if(transfermaker_search.crno !=null && transfermaker_search.crno!="" && transfermaker_search.crno!=null){
        this.searchData.crno=transfermaker_search.crno;
      }
  ...
  clearSearch(){
    ...
    this.transfermarkersearch.controls['crno'].reset('');
    this.first=false;
  ...
  preparefile(){
    let transfer_search = this.transfermarkersearch.value;
    if(this.transfermarkersearch.get('from_date').value==undefined || this.transfermarkersearch.get('from_date').value=='' || this.transfermarkersearch.get('from_date').value==null ){
        this.notification.showWarning("Please Select From date");
        return false;
    }
    if(this.transfermarkersearch.get('to_date').value==undefined || this.transfermarkersearch.get('to_date').value=='' || this.transfermarkersearch.get('to_date').value==null ){
         this.notification.showWarning("Please Select to date");
         return false;
    }
    this.first=true;
    let dates = this.datePipe.transform(transfer_search.from_date, 'yyyy-MM-dd');
    transfer_search.from_date=dates;
    let todate = this.datePipe.transform(transfer_search.to_date,'yyyy-MM-dd');
    transfer_search.to_date=todate;
    if(this.transfermarkersearch.get('capdate_Value').value!=null && this.transfermarkersearch.get('capdate_Value').value !='' && this.transfermarkersearch.get('capdate_Value').value!=null){
      let capdate = this.datePipe.transform(transfer_search.capdate_Value,'yyyy-MM-dd');
      transfer_search.capdate=capdate;
    }
    for(let value in transfer_search){
      if(transfer_search[value]=='' || transfer_search[value]==undefined || transfer_search[value]==null){
        delete transfer_search[value];
      }
    }
    this.spinner.show();
    this.faService.trnsfr_download_prepare(transfer_search).subscribe(result=>{
      this.spinner.hide();
      this.first=false;
  ...
  download_file(){
    ...
    this.spinner.show();
    this.faService.trnsfr_downloadfile().subscribe(
      (response: any) =>{
        this.spinner.hide();
          // this.first=false;
          // this.first=false;
          if (response['type']=='application/json'){
            ...
            let filename:any='Transfer-reort';
