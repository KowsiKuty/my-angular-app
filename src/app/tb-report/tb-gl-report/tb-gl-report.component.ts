...
import { Observable, bindCallback, fromEvent } from 'rxjs';
...
@Component({
  selector: 'app-tb-gl-report',
  templateUrl: './tb-gl-report.component.html',
  styleUrls: ['./tb-gl-report.component.scss'],
  providers:[
...
export class TBGlReportComponent implements OnInit {
  @ViewChild("closeaddpopup") closeaddpopup;
  ppr_env=environment.apiURL
  reportsearch:any;
  TBsummaryObjNew:any;
  tb_gl_SearchForms: boolean;
  tb_gl: boolean=true;
  TB_child_summaryObjNew:any;
  from_date: any;
  to_date: any;
  from_date_balance: any='';
  to_date_balance: any='';
  laksh_amount:any
  dates: any;
  divamount: any;
  ...
  Alei_List=[{"id":1,"flag":"A","name":"Assets"},
  {"id":1,"flag":"L","name":"Liabilities"},
  {"id":1,"flag":"E","name":"Expenses"},
  {"id":1,"flag":"I","name":"Income"}]
  level_List=[{"id":"1","name":"Income"},{"id":"2","name":"HR COST"},{"id":"3","name":"Expense"}
  ]
  ...
  tb_gl_report_list: any = [];
  original_data : any = [];
  has_previous: boolean;
  currentpage: any;
  presentpage: number;
  has_next: boolean=true;
  tb_gl_child_report_list: any;
  original_child_data : any = [];  
  has_child_next: any;
  has_child_previous: any;
  child_presentpage: any;
  child_currentpage: any;
  data_branch_id: any;
  data_subcat_id: any;
  ...
  child_excel_prams: { Amount: any; "Branch Name": any; "Business Name": any; "BS Name": any; "CC Name": any; "GL Name": any; ALEI: any; "Date 2": string; "Date 1": string; };
  arrow_icon_up:boolean = true;
  totalFromBalance: any;
  ...
  constructor(private fb:FormBuilder ,private errorHandler: ErrorhandlingService,private datePipe: DatePipe, public tbservice: TbReportService , private dataService:TbReportService,private vrtshardserv:VertSharedService,private SpinnerService:NgxSpinnerService,private toastr: ToastrService) { 
    ...
    this.glfield = {
      label: "GL Number",
      method: "get",
      url: this.ppr_env + "mstserv/gl_search",
      params: "",
      searchkey: "glno",
      displaykey: "glno",
      // wholedata: true,
    ...
    console.log("branch name",this.Branchfield)
    console.log("gl num",this.glfield)
    this.from_date_balance=""
    this.to_date_balance=""
  }
  currentTime$: Observable<Date>;
  tb_gl_SearchForm:FormGroup;
  formattedDate:any;
  ...
  ngOnInit(): void {
    this.tb_gl_SearchForm=this.fb.group({
      from_date:[],
      to_date:[],
      branch_id:[],
      gl_no_id:[],
    ...
    console.log("Do_permission",this.Do_permission['empbranch_ids'].length,"this.brnach_login_id",this.brnach_login_id)
    console.log("rolessssssssss",this.role_per)
    const unixTimestamp = 1717679328 ;
    this.formattedDate = this.convertTimestamp(unixTimestamp);
...
console.log("this.foemateghghj",this.formattedDate)

  
    this.reportsearch = [{"type":"twodates","fromobj":{label: "From Date",formvalue:"fromdate"},"toobj":{label: "To Date",formvalue:"todate"}}, {"type":"dropdown",inputobj:this.Branchfield,formvalue:"branch_id"},{"type":"dropdown",inputobj:this.glfield,formvalue:"gl"},{"type":"dropdown",inputobj: this.laksh_amount,formvalue:"div_amount" }]
  ...
  TBsummaryData:any  = [{ "columnname": "Branch Name", "key": "branch_name"},
    {"columnname": "Category", "key":"cat_name"} ,  
    {"columnname": "GL Number", "key": "subcat_name",},  
  {"columnname":  this.from_date_balance, "key": "frombalance","type":"Amount","style":{color:"blue",cursor:"pointer"}, function :true , clickfunction:this.TB_from_summary.bind(this,"fromdate")},
  {"columnname":  this.to_date_balance, "key": "tobalance","type":"Amount","style":{color:"blue",cursor:"pointer"}, function :true , clickfunction:this.TB_from_summary.bind(this,"todate")},
  {"columnname": " Difference Amount", "key": "amount","type":"Amount"}
]

TB_child_summaryData:any=[
  { "columnname": "Business Name", "key": "bs_name"},
  {"columnname": "Category Name", "key": "cc_name",},   
...
toda_reset_fun(){ 
    if(this.role_per.name=="Admin"){
      this.tb_gl_SearchForm.controls['business'].reset('')
      this.tb_gl_SearchForm.controls['amount'].reset('')
      this.tb_gl_SearchForm.controls['gl_no_id'].reset('')
      this.tb_gl_SearchForm.controls['branch'].reset('')
      this.tb_gl_SearchForm.controls['branch_do'].reset('')
      this.tb_gl_SearchForm.controls['view'].reset('')
      this.tb_gl_SearchForm.controls['bs_id'].reset('')
      this.tb_gl_SearchForm.controls['cc_id'].reset('')
...
branch_hide(branch){
  ...
  if(branch.id==1){
    ...
    this.branch_name_headers=false;
    this.do_view=false;
    this.tb_gl_SearchForm.controls['branch_id'].reset('')
    this.tb_gl_SearchForm.controls['branch_do'].reset('')
    this.tb_gl_SearchForm.controls['gl_no_id'].reset('')
    this.tb_gl_SearchForm.controls['alei'].reset('')
    this.tb_gl_SearchForm.controls['amount'].reset('')
    this.tb_gl_SearchForm.controls['bs_id'].reset('')
    this.tb_gl_SearchForm.controls['cc_id'].reset('')
    this.tb_gl_SearchForm.controls['business'].reset('')
...
if(branch == "" || branch ==null || branch==undefined){
  ...
  if(this.tb_gl_SearchForm.value.from_date==""|| this.tb_gl_SearchForm.value.from_date==undefined || this.tb_gl_SearchForm.value.from_date==null ){
    this.toastr.warning("Please Select Date 1")
    return false
  }
  if(this.tb_gl_SearchForm.value.to_date==""|| this.tb_gl_SearchForm.value.to_date==undefined || this.tb_gl_SearchForm.value.to_date==null ){
    this.toastr.warning("Please Select Date 2")
    return false
  }
  if(this.role_per.name=="Admin"){
  if(this.tb_gl_SearchForm.value.view == "" || this.tb_gl_SearchForm.value.view == undefined || this.tb_gl_SearchForm.value.view==null){
    this.toastr.warning("Please Select View Type")
    return false
  }
  if(this.tb_gl_SearchForm.value.view.id == 2){
    if(this.tb_gl_SearchForm.value.branch_id == null || this.tb_gl_SearchForm.value.branch_id == undefined || this.tb_gl_SearchForm.value.branch_id == ''){
      this.toastr.warning("Please Select Branch")
      return false
    }
  }
}
  if(event.amount?.id=="C" ){
    this.div_Amt_show="Amount In Crores"
      }else if(event.amount?.id=="A"){
    ...
    let view_name
    if(this.role_per.name=="Admin"){
      view_name=this.tb_gl_SearchForm.value.view?.type??""
      }
      let branch_view_in={"id":2,"name":"Branch Wise","type":"Branch_View"}
      if(this.brnach_login_id ==""){
        if(this.tb_gl_SearchForm.value.branch_do==="" || this.tb_gl_SearchForm.value.branch_do=== null || this.tb_gl_SearchForm.value.branch_do === undefined){

          branch_id=this.tb_gl_SearchForm.value.branch_id?.id??""
        }else{
        if(this.tb_gl_SearchForm.value.branch_id==="" || this.tb_gl_SearchForm.value.branch_id=== null || this.tb_gl_SearchForm.value.branch_id === undefined){
           branch_id=this.tb_gl_SearchForm.value.branch_do?.id??""
        }else{
         branch_id=this.tb_gl_SearchForm.value.branch_do?.id??""
        }
      }
    ...
    if(this.tb_gl_SearchForm.value.business=="" || this.tb_gl_SearchForm.value.business == undefined || this.tb_gl_SearchForm.value.business==null){
      this.tb_gl_SearchForm.controls['bs_id'].reset('')
  this.tb_gl_SearchForm.controls['cc_id'].reset('')
    }
    if(this.tb_gl_SearchForm.value.bs_id == "" || this.tb_gl_SearchForm.value.bs_id == null || this.tb_gl_SearchForm.value.bs_id == undefined){
      this.tb_gl_SearchForm.controls['cc_id'].reset('')
    }
      
  this.from_date_balance=this.datePipe.transform(this.tb_gl_SearchForm.value.from_date, 'yyyy-MM-dd') 
  this.to_date_balance=this.datePipe.transform(this.tb_gl_SearchForm.value.to_date, 'yyyy-MM-dd')
  let gl_id=this.tb_gl_SearchForm.value.gl_no_id?.microsubcatcode??""
  let business_id=this.tb_gl_SearchForm.value.business?.id??""
  this.divamount=this.tb_gl_SearchForm.value.amount?.id??""
  let alei=this.tb_gl_SearchForm.value.alei?.flag??""
  let level=this.tb_gl_SearchForm.value.level?.id??""
  
    let bs_ids=this.tb_gl_SearchForm.value.bs_id?.id??""
  let cc_id=this.tb_gl_SearchForm.value.cc_id?.id??""
  let branch_flags
  if(this.role_per.name=="Admin"){
...
this.page_size=100
  ...
  "bs_id": bs_ids,
  "cc_id":cc_id,
  "subcatunique_no":gl_id,
  "flag": '',
  "page_index": this.page_index,
  "page_size": this.page_size,
  "alei_flag": this.tb_gl_SearchForm.value.alei?.flag??""
}

  this.from_head_date=this.datePipe.transform(this.tb_gl_SearchForm.value.from_date, 'dd-MM-yyyy')
  this.to_head_date=this.datePipe.transform(this.tb_gl_SearchForm.value.to_date, 'dd-MM-yyyy')
  this.xlsx_param={ 
    "Amount" :this.div_Amt_show,
    "Branch Name": this.tb_gl_SearchForm.value.branch_id?.name??"",
    "Business Name": this.tb_gl_SearchForm.value.business?.name??"",
    "BS Name":this.tb_gl_SearchForm.value.bs_id?.name??"",
    "CC Name":this.tb_gl_SearchForm.value.cc_id?.name??"",  
    "GL Name":this.tb_gl_SearchForm.value.gl_no_id?.name??"",
    "ALEI":this.tb_gl_SearchForm.value.alei?.name??"",
 ...
 console.log("param",param)
  this.SpinnerService.show()
    ...
    .subscribe((results: any[]) => {
      ...
      this.amount_division = (event.amount?.id == 'L' ? 100000  : (event.amount?.id == 'C' ? 10000000 : (event.amount?.id == 'A' ? 1 : 100000)))
      let dataPagination = results['pagination'];
      this.tb_gl_report_list=datas 
      this.original_data = [...this.tb_gl_report_list];
      if(this.tb_gl_report_list.length!=0){
        ...
        if(value<0){
          ...
          this.tb_gl_report_list.forEach(item => {
        ...
        // }else{
        //   this.Branch_name_Show=this.tb_gl_report_list[0].branch_name
        // console.log("  this.tb_gl_report_list[0].branch_name",  this.tb_gl_report_list[0].branch_name)
        // }
      }
      else{
  ...
  previousClick(){
    if (this.has_previous === true) {         
      ...
      this.tb_summary("",this.presentpage - 1)
    }
  }
  nextClick(){
...
TB_from_summary(name,data,child_view,page=1){
...
this.tb_gl_SearchForm.controls['cc_id'].reset('')
  ...
  this.data_branch_id=name.branch_id
  this.data_subcat_id=name.microsubcatcode
  let bs_ids=this.tb_gl_SearchForm.value.bs_id?.id??""
  let cc_id=this.tb_gl_SearchForm.value.cc_id?.id??""
  let business_id=this.tb_gl_SearchForm.value.business?.id??""
...
if(data=="BR_View"){
  ...
  param={
    ...
    "bs_id": bs_ids,
    "cc_id":cc_id,
    "subcatunique_no": name.microsubcatcode,
    "flag":'',
...
this.child_excel_prams = {
    "Amount" :this.div_Amt_show,
    "Branch Name": this.tb_gl_SearchForm.value.branch?.name??"",
    "Business Name": this.tb_gl_SearchForm.value.business?.name??"",
    "BS Name":this.tb_gl_SearchForm.value.bs_id?.name??"",
    "CC Name":this.tb_gl_SearchForm.value.cc_id?.name??"",  
    "GL Name":name.subcat_name ? name.subcat_name:"",
    "ALEI":this.tb_gl_SearchForm.value.alei?.name??"",
    "Date 2": this.to_head_date,
    "Date 1": this.from_head_date,
}
...
.subscribe((results: any[]) => {
  this.SpinnerService.hide()
  let datas = results["DATA"];   
  this.tb_gl_child_report_list=datas
  this.tb_gl_child_report_list=datas 
  this.original_child_data = [...this.tb_gl_child_report_list];  
  if(this.tb_gl_child_report_list.length!=0){
    ...
    if(value<0){
      ...
      if(data=="BR_View"){
        let totalFromBalance = 0;
        let totalToBalance = 0;
        this.tb_gl_child_report_list.forEach(item => {
          totalFromBalance += parseFloat(item.frombalance);
          totalToBalance += parseFloat(item.tobalance);
        });
        console.log('Total FromBalance:', totalFromBalance);
...
todate_fun(){
  this.tb_gl_SearchForm.controls['to_date'].reset('')
}
...
branchname() {
  ...
  let branch_flag
    ...
    bran_do=this.tb_gl_SearchForm.value.branch_do?.code??""
    }else{
  ...
  this.tb_gl_SearchForm.get('branch_id').valueChanges
    .pipe(
      debounceTime(100),
...
branch_do_name() {
  ...
  this.tb_gl_SearchForm.get('branch_do').valueChanges
    .pipe(
      debounceTime(100),
      distinctUntilChanged(),
      tap(() => {
    ...
    if (
      ...
      fromEvent(this.matAutocompletebranch_dos.panel.nativeElement, 'scroll')
        ...
        .subscribe(() => {
          ...
          const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
          if (atBottom) {
            if (this.has_next_do_bra === true && !isApiCallInProgress) {
...
gl_number() {
  ...
  this.tb_gl_SearchForm.get('gl_no_id').valueChanges
    .pipe(
      debounceTime(100),
      distinctUntilChanged(),
      tap(() => {
...
public displayfngl(gl?: branchList): string | undefined {
  return gl ? gl.glno +"-"+gl.name: undefined;

}

reset(){
  this.tb_gl_SearchForm.reset()
  this.Branch_data_show_hide=false;
  this.tb_gl_report_list= [];
  this.original_data = [];

}
// previous_child_Click(){
//   if (this.has_child_previous === true) {         
//     this.child_currentpage = this.child_presentpage + 1
//     this.TB_from_summary("",this.child_presentpage - 1)
//   }
// }
// next_child_Click(){
//   if (this.has_child_next === true) {         
//     this.child_currentpage = this.child_presentpage + 1
//     this.TB_from_summary("",this.child_presentpage + 1)
//   }
// }
...
onScroll(){
  ...
  if(this.tb_gl_SearchForm.value.amount.id=="C" ){
    this.div_Amt_show="Amount In Crores"
      }else if(this.tb_gl_SearchForm.value.amount.id=="A"){
        this.div_Amt_show="Amount In Actuals"
      }else{
        this.div_Amt_show="Amount In Lakhs"
      }
...
let business_id=this.tb_gl_SearchForm.value.business?.id??""
  ...
  .subscribe((results: any[]) => {
    ...
    this.tb_gl_report_list = this.tb_gl_report_list.concat(datas);
    this.original_data = [...this.tb_gl_report_list];
    }, error => {
      // this.Branch_data_show_hide=false;
      this.errorHandler.handleError(error);
...
this.tb_gl_SearchForm.controls['cc_id'].reset('')
  ...
  if(this.tb_gl_SearchForm.value.bs_id == "" || this.tb_gl_SearchForm.value.bs_id == null || this.tb_gl_SearchForm.value.bs_id == undefined){
    ...
    let page=this.child_presentpage+1
    let view_name=this.tb_gl_SearchForm.value.view?.id??""
    let param
    let bs_ids=this.tb_gl_SearchForm.value.bs_id?.id??""
    let cc_id=this.tb_gl_SearchForm.value.cc_id?.id??""
    let business_id=this.tb_gl_SearchForm.value.business?.id??""
  ...
  if(this.view_edit=="BR_View"){
   ...
   param={
      ...
      if(counts>0){  
        ...
        .subscribe((results: any[]) => {
        let datas = results["DATA"];               
        this.tb_gl_child_report_list = this.tb_gl_child_report_list.concat(datas);
        this.original_child_data = [...this.tb_gl_child_report_list];  
...
Business_dropdown() {
  ...
  this.tb_gl_SearchForm.get('business').valueChanges
    .pipe(
      debounceTime(100),
      distinctUntilChanged(),
      tap(() => {
...
bsname_dropdown() {
  ...
  this.tb_gl_SearchForm.get('bs_id').valueChanges
    .pipe(
      debounceTime(100),
      distinctUntilChanged(),
      tap(() => {
...
ccname_dropdown() {
  if(this.tb_gl_SearchForm.value.bs_id=="" || this.tb_gl_SearchForm.value.bs_id == undefined || this.tb_gl_SearchForm.value.bs_id ==null){
    this.toastr.warning("Please Select BS")
    return false
   }
  let prokeyvalue: String = "";
  this.getccid(prokeyvalue);
  this.tb_gl_SearchForm.get('cc_id').valueChanges
    .pipe(
      debounceTime(100),
      distinctUntilChanged(),
      tap(() => {
...
public displayalei(cc_name?: branchList): string | undefined {
  return cc_name ? cc_name.name : undefined;
...
Screendownload() {  
  ...
  head_report[1].splice(header_position, 0, 'TB GL Report');
  ...
  XLSX.utils.book_append_sheet(new_workbook, worksheet, 'TB GL Report');
  XLSX.writeFile(new_workbook, "TB_GL_Report.xlsx");
...
Screen_childdownload() {
  ...
  let report_name;
  if (this.branch_wise_data === false) {
      report_name = "Branch Details Report";
  } else {
      report_name = "BSCC Details Report";
  }
  ...
  let header_position = Math.floor(max_length / 2);
  head_report[1].splice(header_position, 0, this.view_edit === "BR_View" ? 'TB GL Branch Report' : 'TB GL BSCC Report');

  let xlsxparam: any = {};
  let xlsx_len = 1;
  for (let xlsx in this.child_excel_prams) {
      console.log("xlsx_param 1=>", xlsx);
      console.log("xlsx_param 2=>", this.child_excel_prams);
      if (this.child_excel_prams[xlsx] !== "") {
          xlsxparam[xlsx] = this.child_excel_prams[xlsx];
      }
  }
  console.log("xlsx=>", xlsxparam);
  if (Object.values(xlsxparam).length !== 0) {
  ...
  const new_workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(new_workbook, worksheet, report_name);
  XLSX.writeFile(new_workbook, report_name + ".xlsx");
  this.arrow_icon_up = true
}


sortData(property: string, order: 'asc' | 'desc'): void {
  this.tb_gl_report_list.sort((a, b) => {
    if (order === 'asc') {
...
sortamount(direction: 'asc' | 'desc', field: 'frombalance' | 'tobalance'): void {
  this.tb_gl_report_list.sort((a, b) => {
    ...
    // Compare based on the selected field and sort order
...
original_dsummary(){
  this.tb_gl_report_list = [...this.original_data];  
}

sort_child_amount(direction: 'asc' | 'desc', field: 'frombalance' | 'tobalance'): void {
  this.tb_gl_child_report_list.sort((a, b) => {
    const aValue = field === 'frombalance' 
      ? a.frombalance / this.amount_division * -1 
      : a.tobalance / this.amount_division * -1;
    const bValue = field === 'frombalance' 
...
original_child_dsummary(){
  this.tb_gl_child_report_list = [...this.original_child_data];  
}
...
backend_download(){
  ...
  let amount_div
  if(this.tb_gl_SearchForm.value.amount?.id=="C" ){
    amount_div=this.tb_gl_SearchForm.value.amount?.id
      }else if(this.tb_gl_SearchForm.value.amount?.id=="A"){
        amount_div=this.tb_gl_SearchForm.value.amount?.id
      }else{
        amount_div="L"
  ...
  let param={
    ...
    "flag":"",
    "type":2,
    "file_name":"TB GL Report",
    "branch_id":branch_id,
    "business_id":this.tb_gl_SearchForm.value.business?.id??"",
    "bs_id":this.tb_gl_SearchForm.value.bs_id?.id??"",
    "cc_id":this.tb_gl_SearchForm.value.cc_id?.id??"",
    "div_amt":amount_div
}
this.SpinnerService.show()
