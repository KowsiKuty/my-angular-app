...
import { MemoService, Category, subCategory, Department } from 'src/app/ememo/memo.service';
...
export class MemoredraftComponent implements OnInit {
  ...
  getForwardFetchData() {
    ...
    this.dataService.getFetch(id)
      .subscribe((data) => {
        ...
        if (senderValue['dept_name']) {
          let deptValues: any = {
            "name": senderValue.dept_name,
            "id": senderId
          }
  ...
  createMemoInput() {
    ...
    if (this.rdo_IOM_nfa_bna === 'Inter-Office Memo') {
      memoclass1.type = 'iom'
    }
    if (this.rdo_IOM_nfa_bna === 'Note for Approval') {
      memoclass1.type = 'nfa'
    }
    if (this.rdo_IOM_nfa_bna === 'Board Notes') {
      memoclass1.type = 'bna'
    }
  ...
  forwardMemoForm() {
    ...
    this.dataService.createForwardForm(this.createMemoInput(), this.images, this.refId)
      .subscribe(res => {
        ...
        this.SpinnerService.hide();
        this.router.navigate(["/ememo/memoView"], { queryParams: { mid: id, from: 'draft', permissionToHandle: true }, skipLocationChange: true });
        return true
      },
        error => {
          this.SpinnerService.hide();
  ...
  saveAsDraft() {
    ...
    // console.log("this.forwardForm.value.sender.id", this.forwardForm.value.sender)
    if (this.forwardForm.value.sender.id === undefined) {
      this.SpinnerService.hide();
      this.toastr.error('Memo Add', 'Invalid From/Sender value', { timeOut: 1500 });
    ...
    if (this.idValue) {
      ...
      this.dataService.reDraftSave(this.redraftCreate(), this.idValue, this.images, this.documentList).subscribe((res) => {
        if (res.code === undefined) {
          ...
          this.dataService.sendMemo(this.idValue).subscribe((res) => {
            ...
            this.SpinnerService.hide();
            this.router.navigate(['/ememo/memoView'], { queryParams: { mid: this.idValue, from: 'memoredraft', MemoView: "YES", permissionToHandle: true }, skipLocationChange: true });
          },
            error => {
              this.SpinnerService.hide();
            });
      ...
      this.dataService.createNewMemo(this.createMemoInput(), this.images).subscribe((result) => {
        if (result.code === undefined) {
          ...
          this.router.navigate(['/ememo/memoView'], { queryParams: { mid: id, MemoView: "YES", permissionToHandle: true }, skipLocationChange: true });
        } else {
          this.SpinnerService.hide();
          this.notification.showError(result.description)
        }
