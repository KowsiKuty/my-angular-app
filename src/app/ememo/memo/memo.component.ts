...
import { MemoService, Department, Category, subCategory } from 'src/app/ememo/memo.service';
...
export class MemoComponent implements OnInit {
  ...
  @ViewChild('autoapprover1') matAutocompleteRec1: MatAutocomplete;
  ...
  ngOnInit(): void {
    this.config = {
      ...
      popover: {
        ...
        link: [["link", ["linkDialogShow", "unlink"]]],
        air: [["font", ["bold", "italic", "underline", "strikethrough"]]],
      },
      ...
      toolbar: [
        ...
        ["table", ["addRow", "addColumn", "deleteRow", "deleteColumn", "deleteTable"]],
      ],
            ...
            click: function () {
              try {
                ...
                if (!context.layoutInfo || !context.layoutInfo.editable) {
                  console.error("Summernote context is not properly initialized.");
                  return;
                }
                ...
                setTimeout(() => {
                  ...
                  if (!range) {
                    console.error("Range is undefined. Ensure the editor is focused.");
                    return;
                  }
    ...
    //                 cell.style.width = "270px";
    //                 cell.style.boxSizing = "border-box";
    //                 cell.textContent = " ";  // ✅ Using textContent instead of innerText
    //               }
    //             }
    ...
    function splitTableForA4Size() {
      const tables = document.querySelectorAll(".note-editable table");
    
      tables.forEach((tableElement) => {
        const table = tableElement as HTMLTableElement;
        if (!table || !table.rows || table.rows.length === 0) return;
    
        const maxColumnsPerTable = 8; // Adjust based on PDF A4 width
        const totalColumns = table.rows[0].cells.length;
        
        if (totalColumns <= maxColumnsPerTable) return; // No need to split
    
        let currentColumnStart = 0;
        while (currentColumnStart < totalColumns) {
          const newTable = document.createElement("table");
          newTable.style.border = "1px solid black";
          newTable.style.width = "100%";
          newTable.style.marginTop = "10px"; // Space between tables
    
          for (let i = 0; i < table.rows.length; i++) {
            const row = table.rows[i];
            const newRow = newTable.insertRow();
    
            for (let j = currentColumnStart; j < Math.min(currentColumnStart + maxColumnsPerTable, totalColumns); j++) {
              if (row.cells[j]) {
                newRow.appendChild(row.cells[j].cloneNode(true)); // Copy cell content
              }
            }
          }
    
          table.parentElement?.appendChild(newTable);
          currentColumnStart += maxColumnsPerTable;
        }
    
        table.remove(); // Remove the original table to avoid duplication
      });
    }
    
    // Call the function after Summernote renders
    setTimeout(() => {
      splitTableForA4Size();
    }, 1000);
    ...
    function calculateColumnsForA4(table) {
      const pageWidth = 700; // Approximate A4 width in pixels
      const columnWidth = table.rows[0]?.cells[0]?.offsetWidth || 100;
      return Math.floor(pageWidth / columnWidth);
    }
  ...
  autocompleteRecScroll() {
    setTimeout(() => {
      if (
        this.matAutocompleteRec1 &&
        this.autocompleteTrigger &&
        this.matAutocompleteRec1.panel
      ) {
        fromEvent(this.matAutocompleteRec1.panel.nativeElement, 'scroll')
          .pipe(
            map(x => this.matAutocompleteRec1.panel.nativeElement.scrollTop),
            takeUntil(this.autocompleteTrigger.panelClosingActions)
          )
          .subscribe(x => {
            const scrollTop = this.matAutocompleteRec1.panel.nativeElement.scrollTop;
            const scrollHeight = this.matAutocompleteRec1.panel.nativeElement.scrollHeight;
            const elementHeight = this.matAutocompleteRec1.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_next === true) {
                this.memoService.get_EmployeeList(this.employeeApproverInput1.nativeElement.value, this.currentpage + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.allEmployeeList = this.allEmployeeList.concat(datas);
  ...
  createMemoInput() {
    ...
    memoclass.type = this.rdo_IOM_nfa_bna;
  ...
  Submit() {
    ...
    if (this.sharedService.Memofrom === 'NFA-MEMO' || this.sharedService.Memofrom === 'BNA-MEMO') {
    ...
    if (this.draftSaveId === undefined) {
      this.dataService.createNewMemo(this.createMemoInput(), this.images).subscribe((res) => {
        ...
        this.router.navigate(['/ememo/memoView'], { queryParams: { mid: id, from: 'memocreate', MemoView: "YES", permissionToHandle: true }, skipLocationChange: true });
      },
        error => {
          this.SpinnerService.hide();
        }
    ...
    if (this.draftSaveId !== '') {
      this.dataService.reDraftSave(this.redraftCreate(), this.draftSaveId, this.images, this.documentList).subscribe((res) => {
        ...
        this.dataService.sendMemo(this.draftSaveId).subscribe((res) => {
          ...
          this.router.navigate(['/ememo/memoView'], { queryParams: { mid: this.draftSaveId, from: 'memocreate', MemoView: "YES", permissionToHandle: true }, skipLocationChange: true });
        },
          error => {
            this.SpinnerService.hide();
          }
  ...
  redraftCreate() {
    ...
    memoclass1.type = this.rdo_IOM_nfa_bna
