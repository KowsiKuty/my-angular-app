...
import { MemoService, Category, subCategory, Department } from "../memo.service";
...
import { map, startWith, finalize, switchMap, debounceTime, distinctUntilChanged, tap ,
...
@Component({
  ...
  styleUrls: ['./memoboard.component.scss'],
  providers: [
...
export class MemoboardComponent implements OnInit {
  ...
  apptype: string[] = ['Pending for Approver/Recommender/Opinion', 'Pending(For Approval)', 'Pending(For Recommendation)', 'Pending (Opinion)', 'Sent by me', 'Received by me', 'Draft', 'Expected e-Memo'];
  urlname = '';
  ApprovalType: any = '';
  ...
  ngOnInit() {
    ...
    let MemoType = localStorage.getItem("MemoTypeBoard")
  ...
  dashboard_datas() {
    ...
    document.querySelectorAll('.modal-backdrop').forEach(b => b.remove());
      ...
      var myModal = new (bootstrap as any).Modal(
        document.getElementById("my-modal-board"),
        {
          backdrop: "static",
          keyboard: false,
        }
  ...
  OnAppTypeChange(val) {
    ...
    if (val) {
      ...
      if (apptype === 'Pending(For Approval)') {
        this.ApprovalType = 'Pending(For Approval)';
        localStorage.setItem("MemoTypeBoard", "type=pending_approval&mtype=bna&view_empid=" + this.empids)
        this.urlname = 'memo?type=pending_approval&mtype=bna&view_empid=' + this.empids;
      }

      if (apptype === 'Sent by me') {
        this.ApprovalType = 'Sent by me';
        localStorage.setItem("MemoTypeBoard", 'filter=' + this.sharedService.loginUserId + '_from_emp&mtype=bna')
        this.urlname = 'memo?filter=' + this.sharedService.loginUserId + '_from_emp&mtype=bna';

      }
      if (apptype === 'Received by me') {
        this.ApprovalType = 'Received by me';
        localStorage.setItem("MemoTypeBoard", 'filter=' + this.sharedService.loginUserId + '_to_emp&mtype=bna')
        this.urlname = 'memo?filter=' + this.sharedService.loginUserId + '_to_emp&mtype=bna';

      }
      if (apptype === 'Draft') {
        this.ApprovalType = 'Draft';
        localStorage.setItem("MemoTypeBoard", 'type=draft&mtype=bna&view_empid=' + this.empids)
        this.urlname = 'memo?type=draft&mtype=bna&view_empid=' + this.empids;

      }

      if (apptype === 'Pending(For Recommendation)') {
        localStorage.setItem("MemoTypeBoard", 'type=pending_approval&mtype=bna&ttype=recommend&view_empid=' + this.empids)
        this.urlname = "memo?type=pending_approval&mtype=bna&ttype=recommend&view_empid=" + this.empids;
        this.ApprovalType = 'Pending(For Recommendation)';

      }

      if (apptype === 'Pending for Approver/Recommender/Opinion') {
        localStorage.setItem("MemoTypeBoard", 'type=pending_approval&mtype=bna&ttype=approver_recommender&view_empid=' + this.empids)
        this.urlname = "memo?type=pending_approval&mtype=bna&ttype=approver_recommender&view_empid=" + this.empids;
        this.ApprovalType = 'Pending for Approver/Recommender/Opinion';
      }
      if (apptype === 'Expected e-Memo') {
        localStorage.setItem("MemoTypeBoard", 'type=pending_approval&mtype=bna&ttype=expected&view_empid=' + this.empids)
        this.urlname = "memo?type=pending_approval&mtype=bna&ttype=expected&view_empid=" + this.empids;
        this.ApprovalType = 'Expected e-Memo';
      }
      if (apptype === 'Pending (Opinion)') {
        localStorage.setItem("MemoTypeBoard", 'type=pending_approval&mtype=bna&ttype=opinion&view_empid=' + this.empids)
        this.urlname = "memo?type=pending_approval&mtype=bna&ttype=opinion&view_empid=" + this.empids;
        this.ApprovalType = 'Pending (Opinion)';
        // return false 
      }
  ...
  empselect(data) {
    ...
    if (this.empids) {
      localStorage.setItem("employee_viewBoard", JSON.stringify(data));
      localStorage.removeItem("memosearch_data");
      localStorage.removeItem("MemoTypeBoard");
      localStorage.removeItem("ls_approvaltype");

      this.urlname = "memo?type=othersview&mtype=bna&view_empid=" + this.empids;
  ...
  OnAppTypeChange1() {
    ...
    if (apptype === 'Pending(For Approval)') {
      this.ApprovalType = 'Pending(For Approval)';
      this.urlname = 'memo?type=pending_approval&mtype=bna';
    }
    if (apptype === 'Sent by me') {
      this.ApprovalType = 'Sent by me';
      this.urlname = 'memo?filter=' + this.sharedService.loginUserId + '_from_emp&mtype=bna';
    ...
    if (apptype === 'Pending(For Recommendation)') {
      this.urlname = "memo?type=pending_approval&mtype=bna&ttype=recommend";
      this.ApprovalType = 'Pending(For Recommendation)';
    }
    if (apptype === 'Pending for Approver/Recommender/Opinion') {
      this.urlname = "memo?type=pending_approval&mtype=bna&ttype=approver_recommender";
      this.ApprovalType = 'Pending for Approver/Recommender/Opinion';
    }
    if (apptype === 'Draft') {
      this.ApprovalType = 'Draft';
      this.urlname = 'memo?type=draft&mtype=bna';
    }
    // if (this.sort_by !== undefined || this.sort_by !== null || this.sort_by != "" ) {
    //   this.urlname = this.urlname + '&sort_by=' + this.sort_by
    // }
  ...
  selectRow(filterId) {
    ...
    this.router.navigate(["/ememo/memoView"], { queryParams: { mid: filterId, from: 'bna', permissionToHandle: true }, skipLocationChange: isSkipLocationChange });
  }
  ...
  Clear($event) {
    localStorage.removeItem("memosearch_data")
    localStorage.removeItem("ls_approvaltype");
    localStorage.removeItem("employee_viewBoard");
      localStorage.removeItem("MemoTypeBoard");
    ...
    if (memo_status === 1) {
      ...
      if (this.ApprovalType == 'Expected e-Memo') {
        this.router.navigate(["/ememo/memoView"], 
          { queryParams: { mid: filterId, MemoView: "YES", permissionToHandle: false,
            empid: this.memoSearchForm?.value?.memoemployee?.id, }, skipLocationChange: isSkipLocationChange });
      } else {
        this.router.navigate(["/ememo/memoView"], { queryParams: { mid: filterId, MemoView: "YES", permissionToHandle: true,
            empid: this.memoSearchForm?.value?.memoemployee?.id, }, skipLocationChange: isSkipLocationChange });
      }
      ...
      this.sharedService.fetchData.next(filterId)
      this.router.navigate(["/ememo/memoRedraft"], { queryParams: { memofrom_rf: 'REDRAFT', permissionToHandle: true,
            empid: this.memoSearchForm?.value?.memoemployee?.id, }, skipLocationChange: isSkipLocationChange });
    }
  }
  employelist: any;
  ...
  private getempCode(pincodekeyvalue) {
    ...
    this.memoService
      ...
      .subscribe((results: any[]) => {
        ...
        this.employelist = datas;
         if (this.employelist && this.employelist.length > 0) {
          let emp = JSON.parse(localStorage.getItem("employee_viewBoard"))
          if(emp != null && 
              emp != undefined && 
              emp != ''){
            this.memoSearchForm
  ...
  ChangeHeaderName(data) {
    ...
    if (data == 'Pending for Approver/Recommender/Opinion' || data == 'Pending(For Approval)' || data == 'Pending(For Recommendation)') {
