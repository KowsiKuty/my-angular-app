...
@Component({
  selector: 'app-newproofingupload',
  templateUrl: './newproofingupload.component.html',
  styleUrls: ['./newproofingupload.component.scss']
})
export class NewproofinguploadComponent  implements OnInit {
  @ViewChild("closeaddpopup") closeaddpopup;
  @ViewChild("proofingfileuploadclose") proofingfileuploadclose;
  ...
  file_view_searchform:FormGroup
  ...
  ngOnInit(): void {
    ...
    // }, 1000 * 20);
     this.fileuploadForm = this.fb.group({
        ...
        this.file_view_searchform=this.fb.group({
file_name:['']
        })
        this.conditiondata={
          id:this.proof_temp_id,
          value:true
        }
  }
  ...
  closedpopup() {
    this.proofingfileuploadclose.nativeElement.click();
    if (this.CommentintervalId) {
      clearInterval(this.CommentintervalId);
    }
    this.CommentintervalId = setInterval(() => {
  ...
  fileuploadsummary(data, pageSize = 1) {
    this.spinner.show();
    this.proofservice.new_file_upload_summary(data, pageSize,this.proof_temp_id).subscribe(
      (results: any) => {
        this.checkedFiles = {};
        this.checkOptions = [];
        this.valueid = [];
  ...
  genfiledownload(data) {
    this.spinner.show();
    this.proofservice.new_Proofing_gen_file(data.id,this.proof_temp_id).subscribe(
      (fullXLS) => {
        console.log(fullXLS);
        let binaryData = [];
        binaryData.push(fullXLS);
  ...
  movefile(TEMP) {
    ...
    if (this.successId) {
      ...
      this.proofservice.new_movefilesummary(data,this.proof_temp_id).subscribe(
        (response) => {
          if (response.status === "Success") {
            this.notification.showSuccess(response.message);
            this.checkedFiles = {};
  ...
  verifyfile() {
    ...
    if (this.successId) {
      ...
      const data = this.selectid;
      this.proofservice.new_verifyfilesummary(data,this.proof_temp_id).subscribe(
        (response) => {
          if (response.status === "SUCCESS") {
            this.notification.showSuccess(response.message);
            this.checkedFiles = {};
  ...
  status_button(){
   ...
   fileuploaddata(data){
    ...
    this.temp_act_id=data.account_id
      }
      fileChange(file,data) {
        ...
        uploadDocument() {
          let template_id: any = this.fileuploadForm.get("accountno").value.id
     
          if(!template_id){
              this.notification.showError("Select template");
              return false;
          }
          if(!this.images){
          ...
          this.spinner.show();
          this.proofservice.new_proofingfileuploadDocument(template_id, this.images,this.proof_temp_id,this.temp_act_id)
        ...
        routetoproofingmapping(){
          this.router.navigate(["/proofing/newproofingmapping"],{ queryParams: { id: this.proof_temp_id },skipLocationChange: true });
        }
        openpopup(data){
          var myModal = new (bootstrap as any).Modal(document.getElementById("viewscreen"), {
            backdrop: "static",
            keyboard: false,
          });
          myModal.show();
        ...
        consolidate_summary_header = [
          ...
          { columnname: "Gen File Name", key: "gen_filename" },
        
          {
            columnname: "Download",
            icon: 'download',"style":{cursor: "pointer",fontWeight:800},
            button: true,
            key: "download",
            function: true,
            clickfunction: this.genfiledownload.bind(this),
        ...
        getsummary(){
          let params = '&file_id='+this.viewid
          let file_name = this.file_view_searchform.get('file_name').value;
          file_name ? (params += "&file_name=" + file_name) : "";
         
          this.get_summary_api = {
            method: "get",
            url: this.ip + "prfserv/proofing_file_search",
            params:params,
          };
        }
        reset_search(){
          this.file_view_searchform.reset();
          this.getsummary()
        }
        deletefile(data){
console.log(data.id)
this.proofservice.deletefile(data.id).subscribe(res=>{
  if(res.code){
    ...
    this.fileuploadsummary('',this.presentpage);
        ...
        report_view(){
          var myModal = new (bootstrap as any).Modal(document.getElementById("download_view"), {
            backdrop: "static",
            keyboard: false,
          });
          myModal.show();
...
this.proofservice.new_proofing_fileupload(data,this.proof_temp_id).subscribe(res=>{
  if(res.code){
    this.notification.showError(res.description)
  }else{
    this.notification.showSuccess(res.message)
        ...
        connecting_file(acc_num){
          this.router.navigate(["/proofing/newconnectionfile"],{ queryParams: { id: this.proof_temp_id ,acc_num : acc_num },skipLocationChange: true });
        }
}
