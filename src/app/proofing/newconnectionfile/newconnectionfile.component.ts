...
@Component({
  selector: 'app-newconnectionfile',
  templateUrl: './newconnectionfile.component.html',
  styleUrls: ['./newconnectionfile.component.scss']
})
export class NewconnectionfileComponent implements OnInit {
  ...
  searchupload(e) {
    ...
    this.fileuploadsummaryObjNew = { "method": "get", "url": this.proofUrl + "prfserv/connection_file_upload_summary", "params": e +"&proof_temp_id="+this.proof_temp_id }
  ...
  constructor(private fb: FormBuilder, private notification: NotificationService, private spinner: NgxSpinnerService, private proofingService: ProofingService,private route: ActivatedRoute,
  ...
  {
    "columnname": "Mapping Status", "key": "mapping_status",validate: true,
    validatefunction: this.statusnames.bind(this),
  },
  {
...
this.filetype = datsa.file_type
  ...
  statusname(data) {
   ...
   statusnames(data) {
    ...
    else if (data.mapping_status == 10) {
      config = {
        class: "table-badge3",
        style:"",
        value: "CM_Updation_Started",
      }
    }
    else if (data.mapping_status == 19) {
      config = {
        class: "table-badge3",
        style: "",
        value: "BM_Started",
    ...
    else if (data.mapping_status == 20) {
      config = {
        class:"table-badge7",
        style: "",
        value: "BM_Updation",
    ...
    else if (data.mapping_status == 21) {
      config = {
        class: "table-badge6",
        style: "",
        value: "BM_Filtering ",
...
// JV
  ...
  uploadDocument() {
    ...
    this.spinner.show();
    this.proofingService.new_connection_uploadDocument(this.images, data,this.proof_temp_id,this.acc_num)
      .subscribe((results: any) => {
        ...
        if (results?.description) {
          ...
          if (results?.status === 'Success' && results?.message === 'Connecting file upload process start') {
  ...
  mapp_connection(){
    ...
    let datss={
      ...
      if(this.fa == "FA-BUCKET"){
        this.proofingService.fa_connection_map(datss,this.proof_temp_id)
        ...
        .subscribe((results: any) => {
          ...
          console.log("UploadFile", results)
          if (results?.status === 'Success' && results?.message === 'Bucket Mapping Starts') {
            this.notification.showSuccess(results?.message);
            // this.onCancel.emit();
            // this.backtouploadsummary()
            this.gl_no=0
        ...
        this.proofingService.fa_maker_connection_map(datss,this.proof_temp_id)
        ...
        .subscribe((results: any) => {
          ...
          console.log("UploadFile", results)
          if (results?.status === 'Success' && results?.message === 'FA Maker Processing start') {
            this.notification.showSuccess(results?.message);
            // this.onCancel.emit();
            // this.backtouploadsummary()
            this.gl_no=0
        ...
        this.proofingService.fa_checker_connection_map(datss,this.proof_temp_id)
        ...
        .subscribe((results: any) => {
          ...
          console.log("UploadFile", results)
          if (results?.status === 'Success' && results?.message === 'FA Checker processing start') {
            this.notification.showSuccess(results?.message);
            // this.onCancel.emit();
            // this.backtouploadsummary()
            this.gl_no=0
        ...
        this.proofingService.connection_map(datss,this.proof_temp_id)
  ...
  backtoscreen(){
    this.router.navigate(["/proofing/newproffingupload"],{ queryParams: { id:this.proof_temp_id },skipLocationChange: true });
  }
  getsummary(){
    this.fileuploadsummaryObjNew = { "method": "get", "url": this.proofUrl + "prfserv/connection_file_upload_summary",params:"&proof_temp_id="+this.proof_temp_id }
  }
}
