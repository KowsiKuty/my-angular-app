...
@Component({
  selector: 'app-add-electricity-details',
  templateUrl: './add-electricity-details.component.html',
  styleUrls: ['./add-electricity-details.component.scss']
})
export class AddElectricityDetailsComponent implements OnInit {
  statedata: any;
  state_has_next = true;
  ...
  ngOnInit(): void {
    ...
    this.premisesid=data.premises_id? data?.premises_id:data?.premise?.id
    ...
    (data.id)?  this.getconsumerdetailsget(data):''
  ...
  onCancelClick() {
    let data = this.shareService.submodulestneb.value;
    data.premises_id? data?.premises_id:data?.premise?.id

    if(data.summarycheck == 1){
      this.router.navigate(['/tneb/electricityexpense/electricitySummary'], { skipLocationChange: true })

    }
    else if(data.summarycheck == 2){
      this.router.navigate(['/tneb/electricityexpense/eleDetailPaymentSummary'], { skipLocationChange: true })
    }
    else {
      this.router.navigate(['/tneb/electricityexpense/elecquerysummary'], { skipLocationChange: true })
  ...
  getconsumerdetailsget(value) {
    ...
    let id=value?.consumer_id? value?.consumer_id:value?.id

    this.spinner.show()
    this.tnebservice.getparticularelectricityconsumer(id).subscribe(
      result => {
        this.spinner.hide()
