...
export class BudgetBuilderComponent implements OnInit {
  ...
  FillName=[
    {id:1,name:'Gl Only'},
    {id:2,name:'Business Wise'},
    {id:3,name:'OverAll Budget'},
    {id:4,name:'Year Of Budget'},
    {id:5,name:'Budget'}
  ]
  ngOnInit(): void {
  ...
  finyear_dropdown() {
    ...
    this.pprSearchForm.get('finyear').valueChanges
      ...
      .subscribe((results: any[]) => {
        ...
        this.pprSearchForm.controls['branchid'].reset('')
  ...
  branchname() {
    ...
    this.finyear= this.pprSearchForm.controls["finyear"].value
    this.Fin_years = this.finyear.finyer
    console.log("Finyear ===>>>", this.Fin_years)
    this.pprSearchForm.get('branchid').valueChanges
      .pipe(
        ...
        switchMap(value => this.dataService.getpprbranchdropdownyear(value, 1, this.Fin_years)
          .pipe(
            finalize(() => {
              this.isLoading = false
  ...
  private getbranchid(prokeyvalue) {
    this.finyear= this.pprSearchForm.controls["finyear"].value
    this.Fin_years = this.finyear.finyer
    this.dataService.getpprbranchdropdownyear(prokeyvalue, 1, this.Fin_years)
      .subscribe((results: any[]) => {
        let datas = results["data"];
        this.branchList = datas;
          ...
          .subscribe(() => {
            ...
            if (atBottom) {
              if (this.has_nextbra === true) {
                this.dataService.getpprbranchdropdownyear(this.branchContactInput.nativeElement.value, this.currentpagebra + 1, this.Fin_years)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
  ...
  Business_dropdown() {
    ...
    this.finyears= this.pprSearchForm.controls["finyear"].value
    this.Fin_yearsbuisness = this.finyears.finyer
    this.pprSearchForm.get('businesscontrol').valueChanges
      .pipe(
        ...
        switchMap(value => this.dataService.getbusinessbudgetdropdownyear(this.sector_id, this.branchid, this.branchcode, value, 1, this.Fin_yearsbuisness)
          .pipe(
            finalize(() => {
              this.isLoading = false
      ...
      if (
        ...
        fromEvent(this.business_name.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            if (atBottom) {
              if (this.has_nextbra === true) {
                this.dataService.getbusinessbudgetdropdownyear(this.sector_id, this.branchid, this.branchcode, this.businessInput.nativeElement.value, this.currentpagebra + 1, this.Fin_yearsbuisness)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
  ...
  private getbusiness(prokeyvalue) {
    ...
    this.dataService.getbusinessbudgetdropdownyear(this.sector_id, this.branchid, this.branchcode, prokeyvalue, 1, this.Fin_yearsbuisness)
      .subscribe((results: any[]) => {
        let datas = results["data"];
        this.businessList = datas;
  ...
  bsname_dropdown() {
    ...
    this.finyearss= this.pprSearchForm.controls["finyear"].value
    this.Fin_yearsbs = this.finyearss.finyer
    this.pprSearchForm.get('bs_id').valueChanges
      .pipe(
        ...
        switchMap(value => this.dataService.getbsbudgetdropdownyear(this.business_id, this.branchid, this.branchcode, value, 1, this.Fin_yearsbs )
          .pipe(
            finalize(() => {
              this.isLoading = false
            }),
  ...
  private getbsid(prokeyvalue) {
    this.finyearss= this.pprSearchForm.controls["finyear"].value
    this.Fin_yearsbs = this.finyearss.finyer
    this.dataService.getbsbudgetdropdownyear(this.business_id, this.branchid, this.branchcode, prokeyvalue, 1, this.Fin_yearsbs )
      .subscribe((results: any[]) => {
        let datas = results["data"];
        this.bsList = datas;
          ...
          .subscribe(() => {
            ...
            if (atBottom) {
              if (this.has_nextbs === true) {
                this.dataService.getbsbudgetdropdownyear(this.business_id, this.branchid, this.branchcode, this.bsInput.nativeElement.value, this.currentpagebs + 1, this.Fin_yearsbs )
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
  ...
  budgetbuildersearechclick(data) {
    ...
    if ((this.pprSearchForm.value.finyear === undefined) || (this.pprSearchForm.value.finyear === null)) {
      ...
      return false;
        ...
        if(typeof this.pprSearchForm.value.finyear =='object'){
          this.fyer = this.pprSearchForm.value.finyear
          let finyear_key=Object.keys(this.pprSearchForm.value.finyear)
          if(finyear_key[0] == 'finyer'){
            this.pprSearchForm.value.finyear = this.pprSearchForm.value.finyear.finyer
          }
          if(finyear_key[0] == 'finyear'){
            this.pprSearchForm.value.finyear = this.pprSearchForm.value.finyear.finyear
          }
        }
        if(typeof this.pprSearchForm.value.finyear =='string'){
          this.fyer = this.pprSearchForm.value.finyear
    ...
    if(this.pprSearchForm.value.flag==undefined || this.pprSearchForm.value.flag == null){
      ...
      this.pprSearchForm.value.flag=typeof this.pprSearchForm.value.flag=="object"?this.pprSearchForm.value.flag.flag :this.pprSearchForm.value.flag
    }
    ...
    if ((data.sectorname == undefined) || (data.sectorname == null)) {
      this.toastr.warning('', 'Please Select Sector', { timeOut: 1500 });


      return false;

    }else if((data.sectorname == "") ){
      this.toastr.warning('', 'Please Select Sector', { timeOut: 1500 });
      return false;
    } else {
      let sector1_name = data.sectorname
      if(typeof this.pprSearchForm.value.sectorname =='object'){
     ...
     }if(typeof this.pprSearchForm.value.sectorname =='string'){
      this.pprSearchForm.value.sector_id = this.pprSearchForm.value.sector_id
      ...
      if (this.pprSearchForm.value.sectorname == undefined) {
        ...
        this.pprSearchForm.value.sector_id = sector1_name
      }
    }
    if ((data.businesscontrol == undefined) || (data.businesscontrol == '')) {
      ...
      this.pprSearchForm.value.business_id = this.pprSearchForm.value.businesscontrol.id
    }
    if ((data.bs_id == undefined) || (data.bs_id == '')) {
      ...
      if(typeof this.pprSearchForm.value.bs_id =='object'){
        this.pprSearchForm.value.bs_name = this.pprSearchForm.value.bs_id.name
        this.pprSearchForm.value.bs_id = this.pprSearchForm.value.bs_id.id
      }if(typeof this.pprSearchForm.value.bs_id == 'string'){
        this.pprSearchForm.value.bs_name = this.pprSearchForm.value.bs_name
        this.pprSearchForm.value.bs_id = this.pprSearchForm.value.bs_id
      }
    }
    if ((data.cc_id == undefined) || (data.cc_id == null)) {
      ...
      if(typeof this.pprSearchForm.value.cc_id=='object'){
        this.pprSearchForm.value.cc_name = this.pprSearchForm.value.cc_id.name
        this.pprSearchForm.value.cc_id = this.pprSearchForm.value.cc_id.id
      }if(typeof this.pprSearchForm.value.cc_id=='string'){
        this.pprSearchForm.value.cc_name = this.pprSearchForm.value.cc_name
        this.pprSearchForm.value.cc_id = this.pprSearchForm.value.cc_id
      }
    }
    ...
    this.searchDatas = {
      ...
      "alei_flag":this.pprSearchForm.value.flag,
      "business_id":this.pprSearchForm.value.business_id,
      "bs_id":this.pprSearchForm.value.bs_id,
      "cc_id":this.pprSearchForm.value.cc_id,
      "microbscode":this.bsmicro_code,
      "microcccode":this.ccmicro_code
    }
    
    this.searchDatas_levels = {
      "entry_module":"AP",  
      "finyear": this.pprSearchForm.value.finyear,
      "year_term": this.type,
    ...
    if (data.tree_flag == 'N') {
      ...
      if (data.Padding_left == '10px') {
        ...
        input_params = {
          ...
          "business_id":this.searchDatas.business_id,
          "bs_id":this.searchDatas.bs_id,
          "cc_id":this.searchDatas.cc_id,
  ...
  addrow_monthly(row_index, pprdata, pprtotaldata, supplierother_task) {
    ...
    if (oatv == false ) {
      ...
      this.toastr.warning('', 'Others has already exist', { timeOut: 1500 });
      return false;
    }
    let ppr_data = []
    ...
    if (ppr_data.length > 0) {
      this.toastr.warning('', "'oops',You are allowed to add only  Others", { timeOut: 1500 });
      return false;
    }
    let a = {
      "Padding_left": "120px",
  ...
  clear_budgetdata() {
    ...
    this.pprSearchForm.controls['file_type'].patchValue(false);
    for (let exp of this.levelslist) {
      exp['expanded'] = false
    }
    this.chipSelectedprodid = []
  ...
  get_all_supplier_key(supplierAll){
    console.log(supplierAll)
    let query = supplierAll
    this.dataService.ppr_getall_supplier(1, query)
    .subscribe((results: any[]) => {
      let datas = results["data"];
      this.supplierAllLists = results["data"];
    })
  ...
  treelevelsDatas(level_index, ind, leveldata, levelsonedatas) {
    ...
    if (leveldata.tree_flag == 'N') {
      ...
      if (leveldata.Padding_left == '50px') {
        ...
        // console.log(levelsonedatas,leveldata,i)
          ...
          level4a = {
            "entry_module":"AP",
            "finyear": this.searchDatas_levels.finyear,
            "year_term": this.type,
            "sector_id": this.searchDatas_levels.sector_id,
            "divAmount": this.amountval_type,
            "business_id": this.searchDatas_levels.business_id,
            "bs_id": this.searchDatas_levels.bs_id,
            "cc_id": this.searchDatas_levels.cc_id,
            "branch_id": this.searchDatas_levels.branch_id,       
            "expensegrp_id":leveldata.expensegrp_id,
            "microbscode":this.searchDatas_levels.microbscode,
            "microcccode":this.searchDatas_levels.microcccode
          }
    ...
    this.SpinnerService.show()
        ...
        this.dataService.expense_list(level_index,level4a, 1)
          .subscribe((results: any[]) => {
  ...
  levelssubcat(level_index, levelexp_index, leveldata, levelsonedatas) {
    var sub_category = {
         ...
         "branch_id": this.searchDatas_levels.branch_id,
          ...
          "divAmount": this.searchDatas_levels.divAmount,
          "expensegrp_id": leveldata.expensegrp_id,
          "subcat_id": leveldata.subcat_id,
          "business_id": this.searchDatas_levels.businesscontrol,
          "sector_id": this.searchDatas_levels.sectorname,
          "year_term": this.searchDatas_levels.year_term,
          "finyear":this.searchDatas_levels.fin_year,
          "bs_id": this.searchDatas_levels.bs_id,
          "cc_id": this.searchDatas_levels.cc_id,
          "microbscode":this.searchDatas_levels.microbscode,
          "microcccode":this.searchDatas_levels.microcccode
    ...
    this.dataService.subexpense_list(level_index,sub_category,1)
      .subscribe((results: any[]) => {
    this.SpinnerService.hide()
  ...
  //     branch_id: this.searchDatas_levels.branch_id,
  //     bs_code: this.searchDatas_levels.bs_id,
  //     cc_code: this.searchDatas_levels.cc_id,
  //     level_id: leveldata.level_id,
  //     expense_id: leveldata.expense_id,
  //     divAmount: this.searchDatas_levels.divAmount,
  //     finyear: this.searchDatas_levels.finyear.finyer,
  //     bizname: this.searchDatas_levels.businesscontrol.name,
  //     sectorname: this.searchDatas_levels.sectorname.name,
  //     yearterm: this.searchDatas_levels.year_term,
  //     cat_id: leveldata.cat_id
  ...
  expant_level(i, level) {
    ...
    if (i == 0) {
      ...
      this.SpinnerService.show()
      this.dataService.getpprdata_level(this.searchDatas_levels).pipe(
        debounceTime(450)
      )
        .subscribe((results: any[]) => {
          this.SpinnerService.hide()
    ...
    if (i == 1) {
      ...
      "sectorname": this.searchDatas.sectorname,
      "divAmount": this.searchDatas.divAmount,
      "bizname": this.searchDatas.masterbusinesssegment_name,
      "bs_code": this.searchDatas_levels.bs_id,
      "cc_name": this.searchDatas_levels.cc_id,
      "branch_id": this.searchDatas_levels.branch_id, 
    }
      ...
      if (this.expand == false) {
        ...
        this.dataService.getdata_level(i,this.searchDatas_levels,"")
          .subscribe((results: any[]) => {
            this.SpinnerService.hide()
    ...
    if (i == 2) {
      ...
      if (this.expand == false) {
        ...
        this.dataService.getdata_level(i,leveltwo,"")
          .subscribe((results: any[]) => {
            this.SpinnerService.hide()
...
excel_download(){
    this.SpinnerService.show();
    this.searchDatas['pagequery']=''
    console.log("seach value=>",this.searchDatas)
    this.dataService.budget_download(this.searchDatas).subscribe((data:any) => { 
      this.SpinnerService.hide();
      ...
      link.download = 'Budget Builder Details'+".xlsx";
      link.click();
  ...
  budget_builder_xlsx(){
    let variance_data = document.getElementById('budgetbuilder');
    const variance_table: XLSX.WorkSheet = XLSX.utils.table_to_sheet(variance_data);
    const new_workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(new_workbook, variance_table, 'Budget Builder');
    XLSX.writeFile(new_workbook, "Budget Builder.xlsx");
  }
  exceldownlode(type){
    let diff=type;
    var data= this.pprSearchForm.value
      if(diff.id==undefined){
        this.toastr.warning("","Please Select The Download Type",{timeOut:1500});
        return false;
      }
      if ((this.pprSearchForm.value.finyear === undefined) || (this.pprSearchForm.value.finyear === null)) {
        this.toastr.warning('', 'Please Select Finyear', { timeOut: 1500 });
    ...
    if(type.name == "Year Of Budget"){
       this.budget_waise={
        "finyear": this.pprSearchForm.value.finyear,
        "alei_flag":this.pprSearchForm.value.flag?.flag,
        ...
        "bsname":this.pprSearchForm.value.bs_id?.name,
        "ccname":this.pprSearchForm.value.cc_id?.name,
        "ccid":this.pprSearchForm.value.cc_id?.id,
        "bsid":this.pprSearchForm.value.bs_id?.id,
        "bizid":this.pprSearchForm.value.businesscontrol?.id,
        "branchid":  this.pprSearchForm.value.branchid?.id,
        "pagequery":"",
        "divAmount": this.pprSearchForm.value.divAmount,
        "overall_file":diff.id == 4 ? 1 : 0,
        "microbscode":this.bsmicro_code,
        "microcccode":this.ccmicro_code
      }
       ...
       this.budget_waise={
        ...
        "sectorname":this.pprSearchForm.value.sectorname,
        "sector_id":this.pprSearchForm.value.sector_id,
        "bizname":this.pprSearchForm.value.masterbusinesssegment_name,
        "bsname":this.pprSearchForm.value.bs_name,
        "ccname":this.pprSearchForm.value.cc_name,
        "ccid":this.pprSearchForm.value.cc_id,
        "bsid":this.pprSearchForm.value.bs_id,
        "bizid":this.pprSearchForm.value.business_id,
        "branchid":  this.pprSearchForm.value.branch_id,
        "pagequery":"",
        "divAmount": this.amountval_type,
        "overall_file":diff.id == 4 ? 1 : 0,
        "microbscode":this.bsmicro_code,
    ...
    this.dataService.budgetbuilder_xlsx(this.budget_waise,type).subscribe(results=>{
  ...
  cc_code(cc){
    this.ccmicro_code = cc.microcccode
  }
  bs_code(bs){
    this.bsmicro_code = bs.microbscode
  }
}
