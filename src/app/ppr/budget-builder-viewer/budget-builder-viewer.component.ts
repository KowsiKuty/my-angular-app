...
export class BudgetBuilderViewerComponent implements OnInit {
  ...
  budgetbuildersearech_click(data) {
    ...
    if ((this.pprSearchForm.value.finyear === undefined) || (this.pprSearchForm.value.finyear === null) || (this.pprSearchForm.value.finyear === '')) {
    ...
    if (this.pprSearchForm.value.year_term === '' || this.pprSearchForm.value.year_term ===null || this.pprSearchForm.value.year_term ===undefined) {
      this.toastr.warning('', 'Please Select Monthly', { timeOut: 1500 });
      this.display = false
    ...
    if(this.pprSearchForm.value.flag==undefined || this.pprSearchForm.value.flag == null || this.pprSearchForm.value.flag== ''){
    ...
    if ((data.sectorname == undefined) || (data.sectorname == null) || (data.sectorname == "")) {
  ...
  treelevel_click(ind, data, data1, mon_type) {
    ...
    if (data.tree_flag == 'N') {
      ...
      if (data.Padding_left == '10px') {
        ...
        // } else {
          this.masterbusinesssegment_name_params = this.pprSearchForm.value.businesscontrol?.name??""
        // }
        let input_params = {
      ...
      if (data.Padding_left == '100px') {
        ...
        // this.exid=
        let input_params = {
          "apinvoicebranch_id": this.pprSearchForm.value.branchid?.id??"",
  ...
  excel_download(){
    ...
    var cc
    var flag
    if(excel.finyear =='' || excel.finyear ==undefined || excel.finyear ==null){
    ...
    if(excel.branchid == '' || excel.branchid == undefined || excel.branchid == null ){
      branch=''
    }else{
      branch=excel.branchid.id
    }
    if(excel.sectorname =='' || excel.sectorname ==undefined || excel.sectorname ==null){
    ...
    if(excel.businesscontrol == '' || excel.businesscontrol == undefined || excel.businesscontrol == null){
      business=''
    }else{
      business=excel.businesscontrol
    }
    if( excel.bs_id == '' ||  excel.bs_id == null || excel.bs_id == undefined){
     bs=''
    }else{
      bs=excel.bs_id
    }
    if( excel.cc_id == '' ||  excel.cc_id == null || excel.cc_id == undefined){
      cc=''
    }else{
      cc=excel.cc_id
    }
  ...
  budget_builder_xlsx(){
    ...
    XLSX.utils.book_append_sheet(new_workbook, variance_table, 'Budget Viewer');
    XLSX.writeFile(new_workbook, "Budget Viewer.xlsx");
  }
}
