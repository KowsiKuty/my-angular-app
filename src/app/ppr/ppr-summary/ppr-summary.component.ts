...
expensegrpList: iDeptList[]; 
  ...
  filename: any;
  index_cat: any;
  exp_level_mapping: boolean;
  income_master: boolean;
  pprexception: boolean;
  pprdocument: boolean;
  dcsdocument: boolean;
  PPR_report_labels: boolean;
  tb_report: boolean;
  ...
  constructor(private errorHandler: ErrorhandlingService,private formBuilder: FormBuilder, private dataService: PprService, private shareService: SharedService,private SpinnerService: NgxSpinnerService,
    public dialog: MatDialog,private toastr:ToastrService,public sharedservices:SharePprService) { }
  PPRMenuList
  ngOnInit(): void {
    // this.SpinnerService.show();
    let datas = this.shareService.menuUrlData.filter(rolename => rolename.name == 'PPR Report');
...
this.SpinnerService.show()
      let index=0
      this.dataService.new_expenselist(index,data,pageNumber)
        .subscribe((results: any[]) => {
this.SpinnerService.hide()
...
let index=0
      ...
      this.dataService.new_cat_list(index,data,1)
        .subscribe((results: any[]) => {
this.SpinnerService.hide()
  ...
  let index=0
        ...
        this.dataService.new_subcatlist(index,data,1)
          .subscribe((results: any[]) => {
  this.SpinnerService.hide()
...
let index=0
    ...
    this.dataService.getsupplieramountdetails(index,data)
      .subscribe((results: any[]) => {
this.SpinnerService.hide()
  ...
  pprsubModuleData(data){
    ...
    if (data.name=='Budget Builder'){
      ...
      this.exp_level_mapping=false
      this.income_master=false
      this.pprexception=false
      this.pprdocument=false;
      this.dcsdocument=false;
      this.PPR_report_labels=false;
      this.tb_report=false;
      // document.getElementById("mySidenav").style.width = "200px";
      // document.getElementById("main").style.marginLeft = "12rem";
      // this.sharedservices.isSideNav = false;
    }
    ...
    if(data.name=="PPR Labels"){
      this.PPR_report_labels=true;
      this.tabcheck_pprreviewer=false;
      this.tabcheck_pprreport=false;
      this.tabcheck_ppr=false
      this.budgetreviewer=false
