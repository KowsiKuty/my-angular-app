...
export class EntryServiceComponent implements OnInit {
  ...
  has_nextTrans=true;
  currentpageTrans: number=1;
  has_previousTrans=true;
 inputValue: string = '';
  addform: FormGroup;
  ...
  task = {
    ...
    color: 'primary',
     subtasks: [
    { name: 'BASEAMOUNT', color: 'primary', checked: false },
    { name: 'IGST', color: 'primary', checked: false },
    { name: 'SGST', color: 'accent', checked: false },
    { name: 'CGST', color: 'accent', checked: false },
    { name: 'INVAMOUNT', color: 'warn', checked: false },
    // { name: 'DEBITAMOUNT', color: 'warn', checked: false }
  ],

  };

  
  @ViewChild('transType') matTranAutocomplete: MatAutocomplete;
  @ViewChild('transInput') tranInput: any;
  constructor(private service: EntryService, private formBuilder:FormBuilder,
    private toastr:ToastrService, private spinner: NgxSpinnerService, private notification: NotificationService,
  ...
  ngOnInit(): void {
    ...
    this.EntrySearchForm.get("transactionTypeList").valueChanges
    .pipe(
      debounceTime(100),
      ...
      switchMap(value => this.service.getEntryTypes(1,value)
        .pipe(
          finalize(() => {
  ...
  public displayTransFn(trans?: transactionLists): string | undefined {
    return trans ? trans.text : undefined;
  }
  
  TransTypesScroll() {
    setTimeout(() => {
      if (
        this.matTranAutocomplete &&
        this.autocompleteTrigger &&
        this.matTranAutocomplete.panel
      ) {
        fromEvent(this.matTranAutocomplete.panel.nativeElement, 'scroll')
          .pipe(
            map(x => this.matTranAutocomplete.panel.nativeElement.scrollTop),
            takeUntil(this.autocompleteTrigger.panelClosingActions)
          )
          .subscribe(x => {
            const scrollTop = this.matTranAutocomplete.panel.nativeElement.scrollTop;
            const scrollHeight = this.matTranAutocomplete.panel.nativeElement.scrollHeight;
            const elementHeight = this.matTranAutocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_nextTrans === true) {
                this.service.getEntryTypes(this.currentpageTrans + 1, this.tranInput.nativeElement.value )
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    if (this.transactionList.length >= 0) {
                      this.transactionList = this.transactionList.concat(datas);
                      this.has_nextTrans = datapagination.has_next;
                      this.has_previousTrans = datapagination.has_previous;
                      this.currentpageTrans = datapagination.index;
                    }
                  })
              }
            }
  ...
  transactionType(data:any,value){
    ...
    this.EntrySearchForm.controls['transactionTypeList'].setValue(data)
...
openDialog(templateRef: any): void {
  this.dialog.open(templateRef, {
    width: '60vw',      // make it wider
    height: 'auto',     // let height adjust based on content
    maxWidth: '80vw',   // optional limit
...
this.spinner.show();
this.service.onformSubmit(payload).subscribe(
      result => {
        this.spinner.hide();
        if (result.code) {
...
reset() {
  const listTransactionDebit = this.debitsave.get('listTransactionDebit') as FormArray;

  listTransactionDebit.controls.forEach((group: FormGroup) => {
    group.get('formula')?.reset('');
  });
...
updateAllComplete(event, data: string, i: number) {
  ...
  if (event.checked) {
    ...
    currentValue += data;
   const found = this.task.subtasks.find(subtask => subtask.name === data);
if (found) {
  found.checked = true;
}
  ...
  .replace(new RegExp(`(^|[+\\-*/])${data}([+\\-*/]|$)`), (match, p1, p2) => {
    if (p1 && p2) return p1;
    return '';
  })
...
onFormulaChange(event: KeyboardEvent) {
  // Captures user typing (+ - * / etc.)
  const input = (event.target as HTMLTextAreaElement).value;
  console.log('Formula updated:', input);
}
