...
export class EntryService {
  ...
  public getEntryTypes(page = 1, data = ''): Observable<any> {
    this.reset();
    const getToken: any = localStorage.getItem('sessionData')
    let tokenValue = JSON.parse(getToken);
    let token = tokenValue.token
    const headers = { 'Authorization': 'Token ' + token }
    let params: any = new HttpParams();
    return this.http.get<any>(url + "entryserv/fetch_entrytype?page=" + page + "&query=" + data, { 'headers': headers, params })
  }
  ...
  public getcatsearchnew(query:any, page:any): Observable<any> {
  ...
  public getcatsearch(query:any, page:any): Observable<any> {
    ...
    let token = tokenValue.token
    const headers = { 'Authorization': 'Token ' + token }
    return this.http.get<any>(url + 'mstserv/Apcategory_search_entry?query=' + query + '&page='+page, { 'headers': headers })
  }
  ...
  public onformSubmit(value): Observable<any> {
    this.reset();
    const getToken: any = localStorage.getItem('sessionData')
    let tokenValue = JSON.parse(getToken);
    let token = tokenValue.token
    const headers = { 'Authorization': 'Token ' + token }
    let params: any = new HttpParams();
    return this.http.post<any>(url + "entryserv/create_entry_type", value,{ 'headers': headers, params })
  }
