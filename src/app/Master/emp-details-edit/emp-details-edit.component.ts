...
export class EmpDetailsEditComponent implements OnInit {
   ...
   @Output() onSubmit=new EventEmitter<any>();
    ...
    ContactType:any={5:'EMPLOYEE',10:'GROPU123',1:'INDIVIDUAL'}
    ...
    ngOnInit(): void {
      ...
      .pipe(
        ...
        switchMap(value => this.masterservice.getbranchdatafilter(value,1)
          .pipe(
            finalize(() => {
              this.isLoading = false
            }),
    ...
    getdata(id:any){
      this.masterservice.getEmployeeedit(id).subscribe(data=>{
        ...
        this.createemployss.get('employeetype').patchValue({'id':data['employee_type'].id,'name':data['employee_type'].name})
        this.createemployss.get('designation').patchValue({'id':data['designation'].id,'name':data['designation'].name});
        ...
        this.createemployss.get('emailid').patchValue(data.email_id);
        this.createemployss.get('supervisor').patchValue({'id':data['supervisor'].id,'full_name':data['supervisor'].name});
        this.createemployss.get('hierarchy').patchValue({'id':data['hierarchy'].id,'layer':data['hierarchy'].name});
        this.createemployss.get('branch').patchValue({'id':data['employee_branch_id'].id,'name':data['employee_branch_id'].name,});
        this.createemployss.get('bsname').patchValue({'id':data['businesssegment'].id,'name':data['businesssegment'].name,});
        this.createemployss.get('ccname').patchValue({'id':data['costcentre'].id,'name':data['costcentre'].name,});
        ...
        this.createemployss.get('pincode').patchValue({'id':data['address']['pincode'].id,'no':data['address']['pincode'].no});
        this.createemployss.get('city').patchValue({'id':data['address']['city'].id,'name':data['address']['city'].city_name});
        this.createemployss.get('district').patchValue({'id':data['address']['district'].id,'name':data['address']['district'].name});
        this.createemployss.get('state').patchValue({'id':data['address']['state'].id,'name':data['address']['state'].name});
        this.createemployss.get('contacttype').patchValue(this.ContactType[data['contact'].type_id]);
        this.createemployss.get('personname').patchValue(data['contact'].name);
        this.createemployss.get('condesignation').patchValue({'id':data['contact']['designation'].id,'name':data['contact']['designation'].name})
        ...
        this.createemployss.get('contactnumber').patchValue(data['contact'].contactnumber);
        this.createemployss.get('contactnumber2').patchValue(data['contact'].contactnumber2);
        this.createemployss.get('conemailid').patchValue(data['contact'].email);
        this.createemployss.get('condob').patchValue(data['contact'].dob);
        this.createemployss.get('conwedday').patchValue(data['contact'].wedding_date);
      });
    }
    ...
    getccdata(){
      ...
      this.masterservice.getccdatafilter(d,'',1).subscribe(data=>{
        this.isLoading=false;
    ...
    employeecreatedata(){
      ...
      let data:any={
        ...
        "employee_branch":this.createemployss.get('branch').value.id,
      ...
      this.masterservice.getlistdepartmentcreate(data).subscribe(datas=>{
        console.log(datas);
        this.spinner.hide();
        if (datas.code === "UNEXPECTED_ERROR") {
          this.notification.showWarning(datas.description)
        } else if (datas.code === "INVALID_DATA") {
          this.notification.showWarning(datas.description)
        }
        else if(datas.code==="INVALID EMPLOYEE ID"){
          this.notification.showWarning(datas.description)
        }  
        else if(datas.code==="INVALID_Address_ID"){
          this.notification.showWarning(datas.description);
        }
        else if(datas.code=== "INVALID Contact_ID"){
          this.notification.showWarning(datas.description)
        }
        else{
