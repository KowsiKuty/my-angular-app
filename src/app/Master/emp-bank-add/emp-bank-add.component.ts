...
export class EmpBankAddComponent implements OnInit {
  ...
  hasnext(){
    if(this.has_next){
      this.getsummary(this.presentPage+1);
    }
  }
  has_previous(){
    if(this.has_pre){
  ...
  getbankdata(){
    ...
    this.masterservice.getempbankdropdown(d,1).subscribe(results=>{
      this.banknamelist=results['data'];
  ...
  this.masterservice.getempbankcreate(d).subscribe(data=>{
    ...
    else if(data.code==='Duplicate Name'){
      this.notification.showWarning(data.description);
    }
    else if(data.code ==='INVALID_DATA'){
      this.notification.showWarning(data.description);
    }
    else if(data.code==='UNEXPECTED_ERROR' && data.description==='NOT ALLOWED TO CREATE EMPLOYEE ACCOUNTDETAILS'){
      this.notification.showError("NOT ALLOWED TO CREATE EMPLOYEE ACCOUNTDETAILS IN 'PROD' ENVIRONMENT")
    }
    else{
