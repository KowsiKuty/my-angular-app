...
@Component({
  selector: 'app-vendor-renewal-report',
  templateUrl: './vendor-renewal-report.component.html',
  styleUrls: ['./vendor-renewal-report.component.scss'],
  providers: [
...
export class VendorRenewalReportComponent implements OnInit {
  ...
  @ViewChild('rejectclose') rejectclose;

  @ViewChild('table') table: ElementRef;   // Import the table element  bug id:
  ...
  yesornodropdown = [
    {
    "options": 'YES', "id": '1','color':'green'
  }, {
    "options": 'NO', "id": '2','color':'red'
  }]
  deptlist: any;
  dep_has_next = true;
  ...
  constructor(private activateroute: ActivatedRoute, private spinnerservice: NgxSpinnerService, private shareService: SharedService, private notification: NotificationService, private atmaService: DataService,
    private errorHandler: ErrorHandlingService, private formbuilder: FormBuilder, public datepipe: DatePipe,private router:Router) { }
  ...
  ngOnInit(): void {
    ...
    //   this.vendorObjs.vendorid = params['datafrom'];
    //   this.vendorObjs.activityid =  params['id'];
    //       })
  ...
  getactivityleveluestion(value) {
    ...
    this.atmaService.evaluateactivitysubmit(value).subscribe(
      result => {
        ...
        let data =result['data'] //bug id
        this.vendor_name = data[0].vendor_name
        this.pan_no = data[0].pan_no

        this.questiondescriptionid=data[0].description_id
        console.log("this.questiondescriptionid===>",this.questiondescriptionid)
        console.log("this.questiondata==>",this.questiondata)
        console.log("results==>",result)
        this.approver=data[0].type_data[0].mappinghtml.type_status.id  
      }

        // if(result[0].code== 'INVALID HEADER ID'){
  ...
  activityevaluation(value) {
    ...
    this.activity = arr[0]?.service_branch?.name;
    this.Remarks= arr[0]?.remarks_data.remarks;
  ...
  getsummaryarray(value) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (arr[i].mapping.length == 0) {
        ...
        arr[i].mappinghtml = {
          type_status:arr[i].mapping[arr[i]?.mapping?.length - 1]?.type_status,
  ...
  rejectquestionnaire(c,a,value,typestatus){
    this.temp_questionnairedata={
      c:c,
      a:a,
      value:value,
      typestatus:typestatus,
    }
    this.questionnaireactionflag=typestatus
  }
  ...
  closereject(){
    ...
    this.remarksvariable=''
  }

  rejectsubmit(num){

    this.questionsubmit(this.temp_questionnairedata.c,this.temp_questionnairedata.a,this.temp_questionnairedata.value,this.temp_questionnairedata.typestatus,num)
  }

  questionsubmit(c, a, value, typestatus,num) {
    ...
    if(typestatus ==2 && +value.tablearray[value?.tablearray?.length-1].tdarray[value.tablearray[value?.tablearray?.length-1]?.tdarray?.length - 1]?.tdvalue == 2 ){
      console.log('last value check if condition',value.tablearray[value?.tablearray?.length-1].tdarray[value.tablearray[value?.tablearray?.length-1]?.tdarray?.length - 1])
      typestatus=3
      console.log('typestatus if condiion',typestatus)
    }

    console.log('last value cehck', +value.tablearray[value?.tablearray?.length-1].tdarray[value.tablearray[value?.tablearray?.length-1]?.tdarray?.length - 1].tdvalue == 2 )
      ...
      for (let j = 0; j < value.tablearray[i].tdarray.length; j++) {
        ...
        if (value.tablearray[i].tdarray[j].is_input && value.tablearray[i].tdarray[j].input_type.name != 'FILE') {
          ...
          let obj = {
            ...
            activity_id: +this.questiondata[c].id,
          ...
          console.log('submitarrayobj===>', obj)
...
// BUG ID:HERE NEED TO DO
    let finalobj = {
      ...
      "remarks": this.remarksvariable,
      "type_status": typestatus,
      "vendor_id": this.vendorid,
      "Activity": +this.questiondata[c].id,
      "answers": submitarray,
      "id": value?.mappinghtml?.id,
      // "dept_id":value?.dept_id,
      "dept_id":this.currentactivityselected?.service_branch?.id,
      // "vendor_code":this.vendordetails?.code+' ('+this.vendordetails?.name+')',
      "vendor_code":this.vendordetails?.name+' ('+this.vendordetails?.code+')',
      "description":value?.description_id?.name,
      "msg":'',
      "level":(typestatus == 1)? 1 :2,
      "activity_id":+this.questiondata[c].id
    };
    (typestatus == 1 )? delete finalobj.id :'';
    console.log(submitarray);
    console.log(finalobj)
    this.yesfinalobj=finalobj;

    ///BUG ID:
      // if(this.questiondescriptionid==38 && this.temp_questionnairedata.value== 6 ){
        // if(this.questiondescriptionid==38 && question.length== 6 ){
          let ansarray=[];
          // if(this.questiondescriptionid==38){
               for(let i=0;i<submitarray.length;i++){
                   if(submitarray[i].answer == ""){
                      ansarray.push("empty");
                   }
               }
      // }
      if(ansarray.includes('empty')){
          this.notification.showError(' be sure to fill all questions'.replace(/\b\w/g, function(l){ return l.toUpperCase() }));
          // return false;
      ...
      if(num==2){
        this.yessubmit(c, a, value, typestatus,finalobj)
        }
        // else{
        //   return
        // }
      ...
      // else{
      //   this.notification.showError('pls be sure to fill all questions'.replace(/\b\w/g, function(l){ return l.toUpperCase() }));
      // }
...
/////////BUG ID:
    ...
    // submitQuestionnaire(){
    //   this.yessubmit(this.temp_questionnairedata.c,this.temp_questionnairedata.a,this.temp_questionnairedata.value,this.temp_questionnairedata.typestatus)

    // }
    yessubmit(c, a, value, typestatus,finalobj) {

      if(this.remarksvariable == "" || this.remarksvariable == undefined || this.remarksvariable == null){
        this.notification.showError("Remarks Cannot Be Empty");
        return false;
      }
      ...
      if(typestatus == 1){
        ...
        this.spinnerservice.show()
        this.atmaService.activitysubmit(finalobj).subscribe(   //here need to do

        result => {
          
          if (result.message == "Successfully Created") {
          this.spinnerservice.hide()
            this.questiondata[c].type_data[a].mappinghtml.type_status.id = -1
            console.log(result)
            this.notification.showSuccess(result.message)
        ...
        this.spinnerservice.show()
      this.atmaService.approveactivitysubmit(finalobj).subscribe(
        result => {
          if(typestatus == 2){
            this.spinnerservice.hide()
          if (result.message == "Successfully Approved") {
            // this.spinnerservice.hide()
            this.questiondata[c].type_data[a].mappinghtml.type_status.id = 2
            this.remarksvariable=''
          ...
          if (result.message == "Successfully Approved") {
            console.log(result)
            this.notification.showSuccess('RENEWAL DECLINED')
            this.questiondata[c].type_data[a].mappinghtml.type_status.id = 3
            this.remarksvariable=''
            this.rejectclose.nativeElement.click()
          }
          else {
