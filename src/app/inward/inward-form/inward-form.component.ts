...
export class InwardFormComponent implements OnInit {
  ...
  has_nextpost = true;
  has_previouspost = true;
  currentpagepost: number = 1;
  postList: Array<postlistss>;
  awb:boolean
  isEnabled: boolean ;
  postDD:boolean
  @ViewChild('post') matpostAutocomplete: MatAutocomplete;
 ...
 ddChannelChange(event) {
  ...
  //   let inwardStatus = !this.editFormsActive && data?.inwardstatus !== 'New';
  //   return inwardStatus;
  // }
  ...
  getEditInward() {
    let data: any = this.shareService.inwardData.value
    this.getChannel();
    console.log("inward edit share",data);
    if(data.inwardstatus === 'New'){
      this.isEnabled = true;
    }
  ...
  courierInwardForm() {
    ...
    if (data.channel == 1) {
      ...
      if (data.post == "" || data.post == null || data.post == undefined) {
        this.notification.showWarning("Please fill Post")
        return false
      }
      if (data.awbno == "" || data.awbno == null || data.awbno == undefined) {
        this.notification.showWarning("Please fill AWB NO")
  ...
  autocompletepostScroll() {
    setTimeout(() => {
      if (
        this.matpostAutocomplete &&
        this.autocompleteTrigger &&
        this.matpostAutocomplete.panel
      ) {
        fromEvent(this.matpostAutocomplete.panel.nativeElement, 'scroll')
          .pipe(
            map(x => this.matpostAutocomplete.panel.nativeElement.scrollTop),
            takeUntil(this.autocompleteTrigger.panelClosingActions)
          )
          .subscribe(x => {
            const scrollTop = this.matpostAutocomplete.panel.nativeElement.scrollTop;
            const scrollHeight = this.matpostAutocomplete.panel.nativeElement.scrollHeight;
            const elementHeight = this.matpostAutocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_next === true) {
                this.dataService.getpostFKdd(this.postInput.nativeElement.value, this.currentpagepost + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.CourierList = this.CourierList.concat(datas);
                    // console.log("emp", datas)
                    if (this.CourierList.length >= 0) {
                      this.has_nextpost = datapagination.has_next;
                      this.has_previouspost = datapagination.has_previous;
                      this.currentpagepost = datapagination.index;
                    }
                  }, (error) => {
                    this.errorHandler.handleError(error);
                    this.SpinnerService.hide();
  ...
  displayFnpost(courier?: postlistss): string | undefined {
    return courier ? courier.name : undefined;
  }
  getpostFK() {
    this.dataService.getpostFKdd("", 1)
      .subscribe((results: any[]) => {
        let datas = results["data"];
        this.postList = datas;
        console.log("CourierList list", datas)
