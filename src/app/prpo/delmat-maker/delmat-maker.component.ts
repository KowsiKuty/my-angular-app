...
export class DelmatMakerComponent implements OnInit {
  ...
  delmatmakerSubmit(){
    if (this.delmatmakerForm.value.employee_id==="" || this.delmatmakerForm.value.employee_id.id== undefined ||this.delmatmakerForm.value.employee_id==null){
      this.toastr.error('Select Employee Field','Empty value inserted' ,{timeOut: 1500});
      return false;
    }if (this.delmatmakerForm.value.commodity_id==="" || this.delmatmakerForm.value.commodity_id.id===undefined || this.delmatmakerForm.value.commodity_id===null){
      this.toastr.error('Select Commodity Field','Empty value inserted' ,{timeOut: 1500});
      return false;
    }
  ...
  .subscribe(res => {
    ...
    else if(res.code === "UNEXPECTED_ERROR" && res.description === "NOT ALLOWED TO CREATE DELMAT"){
      this.SpinnerService.hide();
      this.notification.showError("NOT ALLOWED TO CREATE DELMAT IN 'PROD' ENVIRONMENT")
    }
