...
export class PoReopenComponent implements OnInit {
  ...
  getporeopen(page = 1) {
    ...
    this.dataService.grnproduct(id,page)
      .subscribe(results => {
        ...
        if(this.poreopendataList.length > 0){
  ...
  nextpage(){
    if(this.has_next === true){
      this.getporeopen(this.presentpage + 1)
    }
  }
  previouspage(){
  ...
  reopenid: any
  deliveryReopen(data2,page = 1) {
    this.SpinnerService.show();
    this.dataService.product(this.reopenid, data2,page)
      .subscribe(results => {
        this.SpinnerService.hide();
        let datas = results["data"];
        let datapagination = results.pagination
  ...
  salesweightnextClick() {
    if (this.hasnext === true) {
      this.deliveryReopen(this.presentpg + 1)
    }
  }
