...
export class MEPmakeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataId = data?.id;
    console.log("mepdata", data);
    if (data != "") {
      this.mepStatus = data?.mep_status;
      this.mepContigency = data?.contigency;
      this.isEnable = this.mepStatus != "PENDING-APPROVAL" ? true : false;
      this.getmepedit();
    ...
    this.MEPmakerForm = this.fb.group({
      ...
      startdate: [""],
      //startdate :[''],
      contigency: [{ value: "", disabled: false }, Validators.required],
      enddate: [""],
    ...
    this.MEPmakerForm.get("parno")
      .valueChanges.pipe(
        ...
        switchMap((value) =>
          this.dataService.getparnoFKdd(value, 1).pipe(
            finalize(() => {
    ...
    this.MEPmakerForm.get("branch_id")
      .valueChanges.pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
    ...
    this.MEPmakerForm.get("raisor_id")
      .valueChanges.pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
        ...
        switchMap((value) =>
          this.dataService.getraisorFKdd(value, 1).pipe(
            finalize(() => {
              this.isLoading = false;
            })
          )
    ...
    this.MEPmakerForm.get("projectowner_id")
      .valueChanges.pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
        ...
        switchMap((value) =>
          this.dataService.getprojectownerFKdd(value, 1).pipe(
            finalize(() => {
              this.isLoading = false;
            })
          )
    ...
    this.MEPmakerForm.get("requestfor")
      .valueChanges.pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
        ...
        switchMap((value) =>
          this.dataService.getreqforFK(value, 1).pipe(
            finalize(() => {
              this.isLoading = false;
            })
          )
    ...
    this.MEPmakerForm.get("budgetowner_id")
      .valueChanges.pipe(
        debounceTime(100),
        distinctUntilChanged(),
        tap(() => {
        ...
        switchMap((value) =>
          this.dataService.getbudgetownerFKdd(value, 1).pipe(
            finalize(() => {
              this.isLoading = false;
            })
          )
  ...
  meptotalget() {
    ...
    if (
      this.selectedsValue == undefined ||
      this.selectedsValue == null ||
      this.selectedsValue == ""
    ) {
      return false;
    }
    ...
    this.iscapex = true;
    this.dataService
      .meptotalget(this.selectedValue, this.selectedsValue)
      .subscribe(
        (results: any[]) => {
  ...
  mepdetailsLen = 0;
  getSections() {
    const formArray = this.MEPmakerForm.get("mepdetails") as FormArray;
    this.mepdetailsLen = formArray.length;
    const start = (this.currentPage - 1) * this.pageSize;
    const end = start + this.pageSize;
    return formArray.controls.slice(start, end);
  }
  ...
  onPageChange(pageIndex: number) {
    if (
      pageIndex > 0 &&
      pageIndex <= Math.ceil(this.mepdetailsLen / this.pageSize)
    ) {
  ...
  getTotalPages(): number {
    return Math.ceil(this.mepdetailsLen / this.pageSize);
  }
    ...
    setTimeout(() => {
      if (
        ...
        fromEvent(this.matparnoAutocomplete.panel.nativeElement, "scroll")
          ...
          .subscribe((x) => {
            const scrollTop =
              this.matparnoAutocomplete.panel.nativeElement.scrollTop;
            const scrollHeight =
              this.matparnoAutocomplete.panel.nativeElement.scrollHeight;
            const elementHeight =
              this.matparnoAutocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_next === true) {
                this.dataService
                  .getparnoFKdd(
                    this.parnoInput.nativeElement.value,
                    this.currentpagepar + 1
                  )
                  .subscribe(
                    (results: any[]) => {
                      let datas = results["data"];
  ...
  getparnoFK() {
    ...
    this.dataService.getparnoFK("").subscribe(
      (results: any[]) => {
        this.SpinnerService.hide();
        let datas = results["data"];
        this.parnoList = datas;
  ...
  getbranchFK() {
    this.SpinnerService.show();
    this.dataService.getbranch("").subscribe(
      (results: any[]) => {
        this.SpinnerService.hide();
        let datas = results["data"];
        this.branchList = datas;
    ...
    setTimeout(() => {
      if (
        ...
        fromEvent(this.matraisorAutocomplete.panel.nativeElement, "scroll")
          .pipe(
            map(
              (x) => this.matraisorAutocomplete.panel.nativeElement.scrollTop
            ),
            takeUntil(this.autocompleteTrigger.panelClosingActions)
          )
          .subscribe((x) => {
            const scrollTop =
              this.matraisorAutocomplete.panel.nativeElement.scrollTop;
            const scrollHeight =
              this.matraisorAutocomplete.panel.nativeElement.scrollHeight;
            const elementHeight =
              this.matraisorAutocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_nextraisor === true) {
                this.dataService
                  .getraisorFKdd(
                    this.raisorInput.nativeElement.value,
                    this.currentpageraisor + 1
                  )
                  .subscribe(
                    (results: any[]) => {
                      let datas = results["data"];
  ...
  getraisorFK() {
    this.SpinnerService.show();
    this.dataService.getraisorFK("").subscribe(
      (results: any[]) => {
        this.SpinnerService.hide();
        let datas = results["data"];
        this.raisorList = datas;
  ...
  autocompleteprojectownerScroll() {
    setTimeout(() => {
      if (
        ...
        fromEvent(
          this.matprojectownerAutocomplete.panel.nativeElement,
          "scroll"
        )
          .pipe(
            map(
              (x) =>
                this.matprojectownerAutocomplete.panel.nativeElement.scrollTop
            ),
            takeUntil(this.autocompleteTrigger.panelClosingActions)
          )
          .subscribe((x) => {
            const scrollTop =
              this.matprojectownerAutocomplete.panel.nativeElement.scrollTop;
            const scrollHeight =
              this.matprojectownerAutocomplete.panel.nativeElement.scrollHeight;
            const elementHeight =
              this.matprojectownerAutocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_nextproowner === true) {
                this.dataService
                  .getprojectownerFKdd(
                    this.projectownerInput.nativeElement.value,
                    this.currentpageproown + 1
                  )
                  .subscribe(
                    (results: any[]) => {
                      let datas = results["data"];
                      let datapagination = results["pagination"];
                      this.projectownerList =
                        this.projectownerList.concat(datas);
  ...
  getprojectownerFK() {
    this.SpinnerService.show();
    this.dataService.getprojectownerFK("").subscribe(
      (results: any[]) => {
        this.SpinnerService.hide();
        let datas = results["data"];
        this.projectownerList = datas;
  ...
  displayFnrfor(rfor) {
    ...
    return rfor
      ? this.requestforList.find((x) => x.id === rfor).name
      : undefined;
  }

  getrequestforFK() {
    this.SpinnerService.show();
    this.dataService.getreqfor().subscribe(
      (results: any[]) => {
        this.SpinnerService.hide();
        let datas = results["data"];
        this.requestforList = datas;
      ...
      if (
        ...
        fromEvent(this.matbudgetownerAutocomplete.panel.nativeElement, "scroll")
          .pipe(
            map(
              (x) =>
                this.matbudgetownerAutocomplete.panel.nativeElement.scrollTop
            ),
            takeUntil(this.autocompleteTrigger.panelClosingActions)
          )
          .subscribe((x) => {
            const scrollTop =
              this.matbudgetownerAutocomplete.panel.nativeElement.scrollTop;
            const scrollHeight =
              this.matbudgetownerAutocomplete.panel.nativeElement.scrollHeight;
            const elementHeight =
              this.matbudgetownerAutocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_nextbudjown === true) {
                this.dataService
                  .getbudgetownerFKdd(
                    this.budgetownerInput.nativeElement.value,
                    this.currentpagebudjown + 1
                  )
                  .subscribe(
                    (results: any[]) => {
                      let datas = results["data"];
  ...
  getbudgetownerFK() {
    this.SpinnerService.show();
    this.dataService.getbudgetownerFK("").subscribe(
      (results: any[]) => {
        this.SpinnerService.hide();
        let datas = results["data"];
        this.budgetownerList = datas;
    ...
    setTimeout(() => {
      if (
        ...
        fromEvent(this.matcomAutocomplete.panel.nativeElement, "scroll")
          ...
          .subscribe((x) => {
            const scrollTop =
              this.matcomAutocomplete.panel.nativeElement.scrollTop;
            const scrollHeight =
              this.matcomAutocomplete.panel.nativeElement.scrollHeight;
            const elementHeight =
              this.matcomAutocomplete.panel.nativeElement.clientHeight;
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
  ...
  getcommodityFK() {
    this.SpinnerService.show();
    this.dataService.getcommodityFKkey().subscribe(
      (results: any[]) => {
        this.SpinnerService.hide();
        let datas = results["data"];
        this.commodityList = datas;
  ...
  getmepedit() {
    ...
    this.SpinnerService.show();
    this.dataService.getmepEdit(id).subscribe(
      (result: any) => {
        this.SpinnerService.hide();
        if(result?.code){
        ...
        for (let detail of result.mepdetails) {
          ...
          totalamt.valueChanges.pipe(debounceTime(20)).subscribe((value) => {
            console.log("should be called first");
            this.datasums();
            if (!this.MEPmakerForm.valid) {
              return;
          ...
          this.MEPmakerForm?.get("contigency")
            ?.valueChanges.pipe(debounceTime(20))
            .subscribe((value) => {
              console.log("should be called first");
              this.contigencyvaluechanges();
  ...
  mepmakerSubmit() {
    if (this.isEnable) {
      ...
      let ConAmountValidation =
        amountintotalCon - this.MEPmakerForm.value.amount;
      console.log("amountintotalCon", amountintotalCon);
      console.log("ConAmountValidation", ConAmountValidation);
    }
    if (
      this.MEPmakerForm.value.name == "" ||
      this.MEPmakerForm.value.name == null ||
      this.MEPmakerForm.value.name == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.parno == "" ||
      this.MEPmakerForm.value.parno == null ||
      this.MEPmakerForm.value.parno == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.type == "" ||
      this.MEPmakerForm.value.type == null ||
      this.MEPmakerForm.value.type == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.finyear == "" ||
      this.MEPmakerForm.value.finyear == null ||
      this.MEPmakerForm.value.finyear == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.projectowner_id == "" ||
      this.MEPmakerForm.value.projectowner_id == null ||
      this.MEPmakerForm.value.projectowner_id == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.requestfor == "" ||
      this.MEPmakerForm.value.requestfor == null ||
      this.MEPmakerForm.value.requestfor == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.startdate == "" ||
      this.MEPmakerForm.value.startdate == null ||
      this.MEPmakerForm.value.startdate == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.enddate == "" ||
      this.MEPmakerForm.value.enddate == null ||
      this.MEPmakerForm.value.enddate == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.branch_id == "" ||
      this.MEPmakerForm.value.branch_id == null ||
      this.MEPmakerForm.value.branch_id == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.raisor_id == "" ||
      this.MEPmakerForm.value.raisor_id == null ||
      this.MEPmakerForm.value.raisor_id == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.budgetowner_id == "" ||
      this.MEPmakerForm.value.budgetowner_id == null ||
      this.MEPmakerForm.value.budgetowner_id == undefined
    ) {
    ...
    if (
      this.MEPmakerForm.value.justification == "" ||
      this.MEPmakerForm.value.justification == null ||
      this.MEPmakerForm.value.justification == undefined
    ) {
    ...
    this.MEPmakerForm.get("startdate").setValue(startdate);
    this.MEPmakerForm.get("enddate").setValue(enddate);
    ...
    if (this.isBudgetedReadonly == false || !this.isEnable) {
      if (
        this.MEPmakerForm.value.amount < this.sum ||
        this.MEPmakerForm.value.amount > this.sum
      ) {
        this.toastr.error(
          "Check PCA Details Amount",
    ...
    if(this.mepStatus == 'mod' || this.mepStatus == 'APPROVED')
    ...
    if (!this.isEnable) {
      this.MEPmakerForm.get("contigency").disable();
    }
    ...
    for (let i in dataPCAdetails) {
      ...
      if (Prodcat == "" || Prodcat == undefined || Prodcat == null) {
        this.notification.showWarning(
          "Commodity is not filled, Please check on line " + (indexNumber + 1)
        );
        return false;
      }
      ...
      if (desc == "" || desc == undefined || desc == null) {
        this.notification.showWarning(
          "Description is not filled, Please check on line " + (indexNumber + 1)
        );
        return false;
      }
      ...
      if (amount == "" || amount == undefined || amount == null) {
        this.notification.showWarning(
          "Amount is not filled, Please check on line " + (indexNumber + 1)
        );
        return false;
      }
    }

    let duplicateCheckOnCommodity = this.MEPmakerForm.value["mepdetails"].map(
      (x) => x.productcategory_id.name
    );
    console.log("duplicate check in commodity", duplicateCheckOnCommodity);

    for (let i in duplicateCheckOnCommodity) {
      let first_index = duplicateCheckOnCommodity.indexOf(
        duplicateCheckOnCommodity[i]
      );
      let last_index = duplicateCheckOnCommodity.lastIndexOf(
        duplicateCheckOnCommodity[i]
      );
      ...
      if (first_index !== last_index) {
        ...
        let indexNumber = Number(i);
        this.notification.showWarning(
          "There is a duplicate commodity of '" +
            duplicateCheckOnCommodity[i] +
            "' indentified in line " +
            (indexNumber + 1)
        );
    ...
    for (let i in datas) {
      if (datas) {
        if (datas[i].value.productcategory_id.id == undefined) {
          ...
          datas[i].value.productcategory_id =
            datas[i].value.productcategory_id.id;
        }
      }
    }
    ...
    if (this.MEPmakerForm.value.projectowner_id.id == undefined) {
      ...
      this.MEPmakerForm.value.projectowner_id =
        this.MEPmakerForm.value.projectowner_id.id;
    }
    ...
    if (this.MEPmakerForm.value.budgetowner_id.id == undefined) {
      ...
      this.MEPmakerForm.value.budgetowner_id =
        this.MEPmakerForm.value.budgetowner_id.id;
    }

    let data = this.MEPmakerForm.value;

    if (this.mepheaderId != undefined && this.mepStatus == 'PENDING-APPROVAL') {
      let dataID = Object.assign({}, data, { id: this.mepheaderId });
      this.SpinnerService.show();
      this.dataService.MEPmakerUpdate(dataID).subscribe(
        (result) => {
          this.SpinnerService.hide();
          if (
            ...
            result.error == "Error" &&
            result.description == "Please Check Total Amount"
          ) {
            this.SpinnerService.hide();
            this.clicked = false;
            this.notification.showError(
              "Check PCA amount should not exceed than Selected BPA amount"
            );
          }
          else if (result.code) {
            this.SpinnerService.hide();
      ...
      this.dataService.MEPmakerFormSubmit(data).subscribe(
        (result) => {
          this.SpinnerService.hide();
          if (
            result.code === "INVALID_DATA" &&
  ...
  getParyear() {
    // this.SpinnerService.show();
    this.dataService.getParyear().subscribe(
      (results: any[]) => {
        let datas = results["data"];
        // this.SpinnerService.hide();
  ...
  getParexpensetype() {
    // this.SpinnerService.show();
    this.dataService.getParexpensetype().subscribe(
      (results: any[]) => {
        // this.SpinnerService.hide();
        let datas = results["data"];
  ...
  contigencyvaluechanges() {
    this.convalue =
      (this.MEPmakerForm.value.contigency * this.MEPmakerForm.value.amount) /
      100;
    console.log("check for value changees", this.convalue);
    this.total = (this.MEPmakerForm.value.amount + this.convalue).toFixed(2);
