...
export class PoCancelApprovalComponent implements OnInit {
  ...
  getPocancel(page=1) {
    ...
    let pocancel_status = data?.pocancel_status;
    if(pocancel_status == 'REJECTED' || pocancel_status == 'APPROVED'){
    ...
    this.dataService.grnproduct(id,page)
      .subscribe(results => {
        ...
        if(this.pocanceldataList.length >0){
        ...
        this.totallineAmount = dataamount.reduce((a, b) => a + b, 0);
  ...
  nextpage(){
    if(this.has_next === true){
      this.getPocancel(this.presentpage + 1)
    }
  }
  previouspage(){
