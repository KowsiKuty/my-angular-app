...
private prposhareService: PRPOshareService, private toastr: ToastrService, private notification: NotificationService, ) {
  ...
  ngOnInit(): void {
    ...
    //BUG ID:6569
    // this.mepsummarySearchForm.get('parno').valueChanges
    // .pipe(
    //   debounceTime(100),
    //   distinctUntilChanged(),
    //   tap(() => {
    ...
    // this.mepAppsummarySearch();

    // Listen to changes in "amount"
  this.mepsummarySearchForm.get('amount')?.valueChanges.subscribe(val => {
    if (val !== null && val !== undefined && val !== '') {
      ...
      if (!isNaN(numericValue)) {
        ...
        this.mepsummarySearchForm.get('amount')?.setValue(formatted, { emitEvent: false });
      }
    } 
  }); 

      // Listen to changes in "amount"
  this.mepAppsummarySearchForm.get('amount')?.valueChanges.subscribe(val => {
    if (val !== null && val !== undefined && val !== '') {
      // Always keep numeric value
      const numericValue = val.toString().replace(/,/g, '');
      if (!isNaN(numericValue)) {
        ...
        // Patch formatted string into the input field
        this.mepAppsummarySearchForm.get('amount')?.setValue(formatted, { emitEvent: false });
      }
    } 
  });
  ...
  nextClick() {
    if (this.has_next === true) {
      this.mepsummarySearch(this.presentpage + 1, 10)
  ...
  mepsummarySearch(pageNumber=1,pageSize=10) {
    ...
    this.SpinnerService.show();
    this.dataService.getmepsummarySearch(searchdel,pageNumber)
      .subscribe(result => {
        this.SpinnerService.hide();
        if (result.code) {
          this.SpinnerService.hide();
  ...
  makerexceldownload(){
   ...
   if(data.amount == null || data.amount == undefined || data.amount == ""){
    ...
    if(data.mep_status == null || data.mep_status == undefined || data.mep_status == ""){
      mep_status = ""
    }else{
      mep_status = data.mep_status
    }
    //PCA FILTER 6569
    if(data.parno == null || data.parno == undefined || data.parno == ""){
      mepparno = ""
    }else{
      mepparno = data.parno
    }
    //PCA FILTER 6569
    this.SpinnerService.show();
    this.dataService.getMEPMakerExcel(mepno,mepname,mepamount,mepyear,mepbudget,mep_status,mepparno)
    .subscribe((data) => {
      this.SpinnerService.hide()
      if(data['size']<=75){
...
//6569
    this.SpinnerService.show();
    this.dataService.getMEPCheckerExcel(mepno,mepname,mepamount,mepyear,mepbudget,mep_status,mepparno)
    .subscribe((data) => {
      this.SpinnerService.hide()
      if(data['size']<=75){
        this.toastr.warning("",'Records Not Found',{timeOut:1500})
  ...
  nextClickApp() {
    if (this.has_nextApp === true) {
      this.mepAppsummarySearch(this.presentpage + 1, 10)
    }
  }
  ...
  mepAppsummarySearch(pageNumber=1,pageSize=10) {
 ...
 // 🔹 Remove commas from amount before sending
    ...
    this.SpinnerService.show();
    this.dataService.getmeapproverpsummarySearch(searchdel,pageNumber)
      .subscribe(result => {
        this.SpinnerService.hide();
        console.log("MepList search result", result)
...
meptotalList: any
 ...
 meptotalget() {
    this.selectedsValue = this.mepsummarySearchForm.get('parno').value	
    this.selectedValue = this.mepsummarySearchForm.get('type').value	
