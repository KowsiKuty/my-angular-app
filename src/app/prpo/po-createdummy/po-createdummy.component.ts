...
export class PoCreatedummyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.PRsummarySearchForm.get("commodityname")
      ...
      .valueChanges.pipe(
        ...
        switchMap((value) =>
          this.dataService.getsupplierDropdownFKdd(value, 1).pipe(
            finalize(() => {
              this.isLoading = false;
            }),
            catchError((error) => {
  ...
  previousClickqty() {
    if (this.has_previous_qty === true) {
      this.getdeliveryDetails(this.presentpage_qty - 1);
    }
  }
  nextClickqy() {
  ...
  calcTotalpatch(unitprice, qty, amount, totalamount: FormControl) {
    ...
    let amt = (qtys * unitprices).toFixed(2);
    amount.setValue(+amt, { emitEvent: false });
    this.amtt = amount.value;
    totalamount.setValue(+amt, { emitEvent: false });
    this.ttlamt = totalamount.value;
    this.datasums();
  }
  datasums() {
  ...
  getSuppliersearch() {
    this.SpinnerService.show();
    this.dataService.getsupplierDropdown().subscribe(
      (results: any[]) => {
        let datas = results["data"];
        this.SpinnerService.hide();
        this.suppliersearchList = datas;
  ...
  datadiff() {
    this.amt = this.POForm.value.podetails.map((x) => x.totalamount);
    console.log("data check amt", this.amt);
    this.sum = this.amt.reduce((a, b) => a + b, 0);
    // +this.sum.toFixed(2)
    console.log("sum of total ", this.sum);
  ...
  assetDetails(id) {
    this.SpinnerService.show();
    this.dataService.getAssetDetails(id).subscribe((res) => {
