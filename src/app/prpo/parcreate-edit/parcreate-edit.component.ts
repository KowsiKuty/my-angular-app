    ...
    this.dataService.getparEdit(id)
      .subscribe((result: any) => {
        if (result.code) {
          ...
          this.notification.showError(result.description || "An error occurred");
          return false;
        }
        this.SpinnerService.hide();
        ...
        const { no, year, originalamount, desc, isbudgeted, burstlinewise, burstmepwise, date, content,contigency,id, par_status, remarks} = result;
        ...
        //   id:result.id,
        //   contigency:result.contigency
        // })
        console.log('par status',par_status)
         if(par_status === 'APPROVED' || par_status === 'MODIFICATION-APPROVAL'|| par_status === 'PENDING-APPROVAL' ){
        ...
        this.PARmakerForm.patchValue({
          id, no, year,amount:originalamount, desc, isbudgeted, burstlinewise, burstmepwise,
           date: dates, content,contigency,par_status
        })
        ...
        for (let detail of result.pardetails) {
          ...
          amount.setValue(detail.originalamount);
          perceControl.setValue((this.PARmakerForm.value.contigency * amount.value) / 100);
          perceTotalControl.setValue((amount.value + perceControl.value));
...
if(this.PARmakerForm.value.contigency > this.percentage ){
...
// }
  ...
  //BUG ID:6558-C   // For While changing amount value contigency amount calculation
  ...
  // For While changing contigency value contigency amount calculation
...
//Contigency field validation API
  contivalue(){
...
//     this.toastr.warning('','Contigency is more than 25%',{timeOut:1500});
