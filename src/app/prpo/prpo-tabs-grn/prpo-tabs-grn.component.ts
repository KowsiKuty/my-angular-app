...
export class PrpoTabsGRNComponent implements OnInit {
  ...
  getsuppliername() {
    ...
    this.dataService.getsuppliername(this.suplist, suppliername)
      .subscribe((results) => {
        let datas = results["data"];
        this.SpinnerService.hide();
        this.supplierNameData = datas;
  ...
  subModuleData(data) {
    ...
    if (this.isGRNmaker) {
      ...
      this.isGRNupdateViewTab=false
      this.GRNSummaryFormsearch();
      this.getgrnstatus()
      ...
      this.isGRNViewTab = false
       this.isGRNupdateViewTab=false
      this.grnsummarySearch();
      this.getgrnstatus()
    }
    else if(this.isGRNupdate){
       this.isGRNmakerTab = false
      this.isGRNapproverTab = false
      this.isGRNCreateform = false
      this.isGRNApproverFormTab = false
      this.isGRNViewTab = false
      this.isGRNupdateViewTab=true
  ...
  nextClickgrn() {
    if (this.has_nextgrn === true) {
      ...
      this.GRNSummaryFormsearch(this.presentpagegrn + 1, 10)
    }
  }
  ...
  GRNSummaryFormsearch(pageNumber=1,pageSize=10) {
    ...
    let obj = {
      ...
      grn_status:searchgrn.grn_status
    }
    ...
    this.SpinnerService.show();
    this.dataService.getgrninwardsummarySearch(obj,pageNumber)
      .subscribe(result => {
        this.SpinnerService.hide();
  ...
  dataclear() {
    ...
    this.GRNDataForm.controls['grn_status'].reset("")
    this.GRNSummaryFormsearch();
  }
  only_char(event) {
    var a;
  ...
  nextClick() {
    if (this.has_next === true) {
      this.grnsummarySearch(this.presentpage + 1, 10)
    }
  }
  ...
  resetgrnapp() {
    ...
    this.grnSearchForm.controls['grn_status'].reset("")
  ...
  grnsummarySearch(pageNumber=1,pagesize=10) {
    ...
    this.SpinnerService.show();
    this.dataService.grnsummarySearch(obj,pageNumber)
      .subscribe(result => {
        this.SpinnerService.hide();
        let datas = result["data"];
        console.table("grnsum", datas);
  ...
  makerexceldownload(){
      ...
      if(data.grn_status == null || data.grn_status == undefined ||  data.grn_status == ""){
        grn_status = ""
      }else{
    ...
    //   }
      ...
      this.SpinnerService.show();
      this.dataService.getGRNMakerExcel(no,supid,fromdate,todate,branchid,grn_status)
      .subscribe((data) => {
        this.SpinnerService.hide()
        if(data['size']<=75){
  ...
  //   }
    ...
    this.SpinnerService.show();
    this.dataService.getGRNApproverExcel(no,supid,fromdate,todate,branchid,grn_status)
    .subscribe((data) => {
      this.SpinnerService.hide()
      if(data['size']<=75){
        this.toastr.warning("",'Records Not Found',{timeOut:1500})
