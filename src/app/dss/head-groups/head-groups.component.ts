...
export class HeadGroupsComponent implements OnInit {
  ...
  sourcehas_next: boolean;
  sourcehas_previous: boolean;
  sourcecurrentpage: number;
  has_nextsource: any;
  has_previoussource: boolean;
  currentpagesource: number;
  currentpagesummary: number;
  presentpage_summary: any;
  ...
  source_dropdown() {
   ...
   autocompletesourceScroll(){
    this.sourcehas_next = true;
    this.sourcehas_previous = true;
      ...
      if (
        ...
        fromEvent(this.sourceauto.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            if (atBottom) {
              if (this.sourcehas_next === true) {
                this.dataService.get_source_dropdown(this.sourceinput.nativeElement.value, this.sourcecurrentpage + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.source_data = this.source_data.concat(datas);
                    if (this.source_data.length >= 0) {
                      this.sourcehas_next = datapagination.has_next;
                      this.sourcehas_previous = datapagination.has_previous;
                      this.sourcecurrentpage = datapagination.index;
                    }
                  })
              }
            }
  ...
  headgrp_summary_search(headgrpdata,pageNumber=1, pageSize=10){
    ...
    this.dataService.summary_headgrp_view(this.headgrp_param,pageNumber,pageSize).subscribe((results:any)=>{
      ...
      if (this.headgrp_summary.length > 0) {
        ...
        this.presentpage_summary = dataPagination.index;
  ...
  previousClick(){
    ...
    if (this.has_previous_summary === true) {
      ...
      this.currentpagesummary = this.presentpage_summary + 1
      this.headgrp_summary_search(this.headgrp_param,this.presentpage_summary - 1, 10)
    }
  }
...
EditSource(editheader,i){
  console.log("Edit source:", editheader)
...
savesource(saveheader,i){
  console.log("Savehead:", saveheader)
...
deletehead(dssdelete,i){
  ...
  autocompletesourcetoScroll(){
    this.has_nextsource  = true;
    this.has_previoussource  = true;
    this.currentpagesource  = 1
      ...
      if (
        ...
        fromEvent(this.addsourceauto.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_nextsource  === true) {
                this.dataService.get_source_dropdown(this.sourceinputto.nativeElement.value, this.currentpagesource + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.sourcedata = this.sourcedata.concat(datas);
                    if (this.sourcedata.length >= 0) {
                      this.has_nextsource = datapagination.has_next;
                      this.has_previoussource  = datapagination.has_previous;
                      this.currentpagesource  = datapagination.index;
                    }
                  })
              }
            }
  ...
  save_source(headgrpdata,paramfilter){
    console.log("Save source:", headgrpdata)
