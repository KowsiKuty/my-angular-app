...
export class SubGroupsComponent implements OnInit {
  ...
  has_nextheadgrpScrolladd: boolean;
  has_previousheadgrpScrolladd: boolean;
  currentpageheadgrpScrolladd: number;
  subgrpScrollhas_next: boolean;
  subgrpScrollhas_previous: boolean;
  subgrpScrollcurrentpage: number;
  headgrpScrollhas_next: boolean;
  headgrpScrollhas_previous: boolean;
  headgrpScrollcurrentpage: number;
  ...
  autocompletesubgrpScroll() {
    this.subgrpScrollhas_next = true;
    this.subgrpScrollhas_previous = true;
    this.subgrpScrollcurrentpage = 1
      ...
      if (
        ...
        fromEvent(this.matAutocompletesubgrp.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            if (atBottom) {
              if (this.subgrpScrollhas_next === true) {
                this.dataService.get_subgrp_dropdown(this.subgrpinput.nativeElement.value, this.subgrpScrollcurrentpage + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.subgrp_data = this.subgrp_data.concat(datas);
                    if (this.subgrp_data.length >= 0) {
                      this.subgrpScrollhas_next = datapagination.has_next;
                      this.subgrpScrollhas_previous = datapagination.has_previous;
                      this.subgrpScrollcurrentpage = datapagination.index;
                    }
                  })
              }
            }
  ...
  autocompleteheadgrpScroll() {
    this.headgrpScrollhas_next = true;
    this.headgrpScrollhas_previous = true;
    this.headgrpScrollcurrentpage = 1
      ...
      if (
        ...
        fromEvent(this.matAutocompleteheadauto.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.headgrpScrollhas_next === true) {
                this.dataService.get_headgrp_dropdown(this.headgrpinput.nativeElement.value, this.headgrpScrollcurrentpage + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.headgrp_data = this.headgrp_data.concat(datas);
                    if (this.headgrp_data.length >= 0) {
                      this.headgrpScrollhas_next = datapagination.has_next;
                      this.headgrpScrollhas_previous = datapagination.has_previous;
                      this.headgrpScrollcurrentpage = datapagination.index;
                    }
                  })
              }
            }
  ...
  subgrp_summary_search(subgrp_data,pageNumber=1){
    console.log("subgroup summary values:", subgrp_data)
  ...
  savesubgrp(subgrp,i){
    console.log("Save subgroup", subgrp)
  ...
  deletesubgroup(dssdelete,i){
    console.log("delete values:", dssdelete)
  ...
  Editsubgrp(subgepedit,i){
    console.log("Edit values:", subgepedit)
  ...
  autocompleteheadgrpScrolladd() {
    this.has_nextheadgrpScrolladd = true;
    this.has_previousheadgrpScrolladd = true;
    this.currentpageheadgrpScrolladd = 1
      ...
      if (
        ...
        fromEvent(this.matAutocompleteheadtoauto.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.has_nextheadgrpScrolladd === true) {
                this.dataService.get_headgrp_dropdown(this.headgrpinput.nativeElement.value, this.currentpageheadgrpScrolladd + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.headgrpdata = this.headgrpdata.concat(datas);
                    if (this.headgrpdata.length >= 0) {
                      this.has_nextheadgrpScrolladd = datapagination.has_next;
                      this.has_previousheadgrpScrolladd = datapagination.has_previous;
                      this.currentpageheadgrpScrolladd = datapagination.index;
                    }
                  })
              }
            }
