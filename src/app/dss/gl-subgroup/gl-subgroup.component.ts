...
export class GlSubgroupComponent implements OnInit {
  ...
  has_previoussummary: any;
  has_nextsummary: any;
  presentpagesummary: any;
  subgrp_dropdownhas_next: boolean;
  subgrp_dropdownhas_previous: any;
  subgrp_dropdowncurrentpage: any;
  glsubgrp_dropdownhas_next: boolean;
  glsubgrp_dropdownhas_previous: boolean;
  glsubgrp_dropdowncurrentpage: number;
  currentpagesummary: any;
  ...
  autocompletegl_subgrpScroll() {
    this.glsubgrp_dropdownhas_next = true;
    this.glsubgrp_dropdownhas_previous = true;
    this.glsubgrp_dropdowncurrentpage = 1
      ...
      if (
        ...
        fromEvent(this.matAutocompleteglsubgrpauto.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            if (atBottom) {
              if (this.glsubgrp_dropdownhas_next === true) {
                this.dataService.get_glsubgrp_dropdown(this.gl_subgrpinput.nativeElement.value, this.glsubgrp_dropdowncurrentpage + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.gl_subgrp_data = this.gl_subgrp_data.concat(datas);
                    if (this.gl_subgrp_data.length >= 0) {
                      this.glsubgrp_dropdownhas_next = datapagination.has_next;
                      this.glsubgrp_dropdownhas_previous = datapagination.has_previous;
                      this.glsubgrp_dropdowncurrentpage = datapagination.index;
                    }
                  })
              }
            }
  ...
  gl_subgrp_summary_search(gl_subgrp_data,pageNumber=1){
    console.log("GL Subgroup summary values:", gl_subgrp_data)
    ...
    this.SpinnerService.hide()
      ...
      if (this.gl_subgrp_summary.length > 0) {
        ...
        this.has_nextsummary = dataPagination.has_next;
        this.has_previoussummary = dataPagination.has_previous;
        this.presentpagesummary = dataPagination.index;
        ...
        console.log("val1")
        this.has_nextsummary = false;
        this.has_previoussummary=false;
  ...
  previousClick(){
    if (this.has_previoussummary === true) {
         
      this.currentpagesummary = this.presentpagesummary + 1
      this.gl_subgrp_summary_search(this.gl_subgrp_pram,this.presentpagesummary - 1)
    }
  }
  nextClick(){
    if (this.has_nextsummary === true) {
  ...
  Cancelglsubgrp(dssform, i) {
    ...
    console.log(control)
    if(dssform.value.glsubgroups[i].glid  != ""){
      console.log('true')
...
saveglsubgrp(gl_subgroup,i){
  console.log("Save GL subgroup:", gl_subgroup)
  
  let gl_subgrp=gl_subgroup.value.glsubgroups[i]
  if(gl_subgrp.glsubgrp=='' || (typeof gl_subgrp.glsubgrp!='object')){
    this.toastr.warning('', 'Please Select Sub-Group', { timeOut: 1500 });

    return false;
  }  
  if(gl_subgrp.glname=='' || gl_subgrp.glname==undefined || gl_subgrp.glname== null){
...
deleteglsubgrp(deletegl,i){
  ...
  autocompleteglsubgrpScroll() {
    this.subgrp_dropdownhas_next = true;
    this.subgrp_dropdownhas_previous = true;
    this.subgrp_dropdowncurrentpage = 1
      ...
      if (
        ...
        fromEvent(this.matAutocompletegl_subgrpauto.panel.nativeElement, 'scroll')
          ...
          .subscribe(() => {
            ...
            const atBottom = scrollHeight - 1 <= scrollTop + elementHeight;
            if (atBottom) {
              if (this.subgrp_dropdownhas_next === true) {
                this.dataService.get_subgrp_dropdown(this.glsubgrpinput.nativeElement.value, this.subgrp_dropdowncurrentpage + 1)
                  .subscribe((results: any[]) => {
                    let datas = results["data"];
                    let datapagination = results["pagination"];
                    this.glsubgrp_data = this.glsubgrp_data.concat(datas);
                    if (this.glsubgrp_data.length >= 0) {
                      this.subgrp_dropdownhas_next = datapagination.has_next;
                      this.subgrp_dropdownhas_previous = datapagination.has_previous;
                      this.subgrp_dropdowncurrentpage = datapagination.index;
                    }
                  })
              }
            }
