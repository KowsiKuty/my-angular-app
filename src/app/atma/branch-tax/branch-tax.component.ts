...
export class BranchTaxComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.ParentObj= { label: "Tax", "method": "get", "url": this.atmaUrl + "mstserv/tax_search" , params: "", searchkey: "query", displaykey: "name", required: true,
      formcontrolname: 'tax',
    }
    this.ChildObj= { label: "Sub Tax", "method": "get", "url": this.atmaUrl + "mstserv/subtax_search" , params: "", searchkey: "query" ,"displaykey":"name","Depkey":"id","DepValue": "tax_id" , required: true,
      formcontrolname: 'subtax',
    }
    this.taxratedata= {label: "Tax Rate ", "method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params:''  ,wholedata: true, searchkey: "query", displaykey: "rate", required: true,
  ...
  //       switchMap(value => this.atmaService.Taxrate_dropdownscroll(this.subtaxid, value,this.vendordata.panno, 1)
  //         .pipe(
  //           finalize(() => {
  //             this.isLoading = false
  //           }),
  ...
  //   let ss=(this.vendordata.panno=='') ? "false":'True'
  //   if(this.tax_rate_boo && this.inn<=1
  //   ){
  //     this.taxratedata= {label: "Tax Rate ", "method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params:'&subtax_id=' + id+'&pan_no='+ss  ,wholedata: true, searchkey: "query", displaykey: "rate",defaultvalue:this.tax_rate? this.tax_rate :'' ,required: true}
  //     this.inn+=1
  //   }
  //   else{
  //     this.taxratedata= {label: "Tax Rate ", "method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params:'&subtax_id=' + id+'&pan_no='+ss  ,wholedata: true, searchkey: "query", displaykey: "rate" ,required: true}

  //   }

  //   this.atmaService.Taxrate_dropdownscroll(id, taxratekeyvalue,this.vendordata.panno, 1)
  //     .subscribe((results: any[]) => {
  //       let datas = results["data"];
  //       this.taxratelist = datas;
  ...
  //           if (atBottom) {
  //             if (this.has_next === true) {
  //               this.atmaService.Taxrate_dropdownscroll(this.subtaxid, this.taxrateInput.nativeElement.value,this.vendordata.panno, this.currentpage + 1)
  //                 .subscribe((results: any[]) => {
  //                   let datas = results["data"];
  //                   let datapagination = results["pagination"];
  ...
  brantax_datalink() {
    ...
    this.atmaService.Branchtaxeditget(data.id, this.vendordata.id).subscribe(res => {
      console.log("this.ressss---", res)
      this.subtaxid = res.subtax.id
      this.taxtratevalue = res
      this.tax_rate=res.taxrate
      this.tax_id = res.tax.id
      this.pan_num = res.panno
      let ss=(this.pan_num=='') ? "false":'True'
      if(this.tax_id != undefined || this.tax_id != null){
      // this.ChildObj= { label: "Sub Tax", "method": "get", "url": this.atmaUrl + "mstserv/subtax_search" , params: "tax_id=" +this.tax_id, searchkey: "query" ,"displaykey":"name", required: true,
      //   
      // }
      // if(this.subtaxid != undefined || this.subtaxid != null){
      // this.taxratedata= {label: "Tax Rate ", "method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params:'&subtax_id=' + this.subtaxid+'&pan_no='+ss ,wholedata: true, searchkey: "query", displaykey: "rate", required: true,
      //   
      // }
      // }
     }
      this.ParentObj = { label: "Tax",formcontrolname: 'tax', "method": "get", "url": this.atmaUrl + "mstserv/tax_search" , params: "", searchkey: "query", displaykey: "name",  required: true}
       this.ChildObj = { label: "Sub Tax",formcontrolname: 'subtax', "method": "get", "url": this.atmaUrl + "mstserv/subtax_search" , params: "", searchkey: "query" ,"displaykey":"name","Depkey":"id","DepValue": "tax_id" , required: true}
      this.taxratedata= {label: "Tax Rate ", formcontrolname: 'taxrate',"method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params: "&subtax_id=" + this.subtaxid + "&pan_no=" + this.vendordata.panno  ,wholedata: true, searchkey: "query", displaykey: "rate", required: true}
      this.tax_rate_boo=true
      // this.frtodate = {"fromobj":{label: "Valid From Date", defaultvalue: res.excemfrom, required: true,},toobj:{label: "Valid To Date",  defaultvalue: res.excemto, required: true}}
      let msme = res.msme
      ...
      if(res.excemfrom =="None" && res.excemto=="None"){
   ...
   if(this.taxratefield){
    if (this.taxform.value.taxrate == "" ||  this.taxform.value.taxrate==null || this.taxform.value.taxrate.id == undefined ) { 
      ...
      if (this.taxform.value.excemthrosold === "") {
        this.toastr.error("Please Enter Excemthrosold ")
        this.SpinnerService.hide();
        return false
    ...
    this.fileToUpload.append("data", JSON.stringify(this.taxarray))
  ...
  selectedvalues(partdata){
    ...
    // this.taxform.get('taxrate').setValue(null);
    // this.displayFntaxrate(null)
    this.taxid = partdata.id;
    // this.SubTax_dropdownget(this.taxform.value.tax.id, '')
  }
  taxparentchilddata(id, taxratekeyvalue){
    let ss=(this.vendordata.panno=='') ? "false":'True'
    //  if(this.tax_rate_boo){
      // this.taxratedata= {label: "Tax Rate ", "method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params:'&subtax_id=' + id+'&pan_no='+ss  ,wholedata: true, searchkey: "query",required: true,formcontrolname: 'taxrate',}
      // this.inn+=1
    //  }
    // else{
     this.taxratedata= {label: "Tax Rate ", "method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params:'&subtax_id=' + id+'&pan_no='+ss  ,wholedata: true, searchkey: "query", displaykey: "rate" ,required: true,formcontrolname: 'taxrate',}

  //  }
  }
 
  selectedvaluess(childata){
    if(childata!='' &&childata !=undefined && childata!=null ){
      this.subtaxid = childata.id
      this.taxtratevalue = childata
      console.log("childata0", childata)
  // this.taxform.patchValue({
  //   subtax: childata
  // })
  this.taxform.get('taxrate').setValue(null);
  ...
  this.taxparentchilddata(this.taxform.value.subtax.id, '')
    }
    
// if (childata !== "" && childata !== undefined && childata !== null ){
//   this.taxratedata= {label: "Tax Rate ", "method": "get", "url": this.atmaUrl + "mstserv/taxrate_search" , params: "&subtax_id=" + this.subtaxid + "&pan_no=" + this.vendordata.panno  ,wholedata: true, searchkey: "query", displaykey: "rate"}
// }
  }

  taxrateclick(taxratefun){
    console.log("taxratefun",taxratefun)
    // this.taxform.patchValue({
    //   taxrate: taxratefun
    // })
  }
...
// })
// }
// ParentObj:any={ label: "Tax"}
// ChildObj:any={ label: "Sub Tax"}
ParentObj:any
ChildObj:any
inn:number=0
taxratedata:any
frtodate:any = {"fromobj":{label: "Valid From Date", required: true,formcontrolname:"excemfrom"},toobj:{label: "Valid To Date", required: true,formcontrolname:"excemto"}}
