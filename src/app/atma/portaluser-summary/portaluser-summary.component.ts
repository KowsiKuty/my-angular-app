...
export class PortaluserSummaryComponent implements OnInit {
  ...
  getusersummary(){
    this.atmaservice.getportalusersummary(this.vendordetails.portal_code)
    .subscribe(res =>{
  ...
  portalsubmit(){
    ...
    this.atmaservice.createportal(this.vendordetails?.portal_code,this.portalform.value).subscribe(
      res=>{
    ...
    // this.atmaservice.userCreationForm(this.portalform.value,this.portalid).subscribe(
    //   result=>{
