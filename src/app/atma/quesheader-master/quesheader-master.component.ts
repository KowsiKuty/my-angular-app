...
export class QuesheaderMasterComponent implements OnInit {
  ...
  submitqueshdrForm(){
    ...
    for(let i=0;i<values.length;i++){
      if( values[i].type_id=='' || values[i].type_id==null || values[i].type_id==undefined){
        this.notification.showError('Please select Category')
        return false
      }
      if(values[i].name == '' || values[i].name == undefined || values[i].name == null){
        this.notification.showError('Please enter Name')
        return false

      }
      if(values[i].is_input && (values[i].input_type == '' || values[i].input_type == null || values[i].input_type == undefined) ){
        this.notification.showError('Please Select Input Type')
        return false

      }

      if(values[i].order == '' || values[i].order == undefined || values[i].order == null){
        this.notification.showError('Please enter Order')
        return false

      }
      console.log(i,values[i].type_id,values[i].name,values[i])
    }
