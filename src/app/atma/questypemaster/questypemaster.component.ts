...
export class QuestypemasterComponent implements OnInit {
  ...
  submitquesForm() {
    const value = this.QuesTypeaAddForm.value.questypeheader
   

    for(let i=0;i<value.length;i++){
      if (value[i].name == '' || value[i].name == null || value[i].name == undefined) {
        this.notification.showError("Please Enter Name");
        return false
      }
      if (value[i].remarks == '' || value[i].remarks == null || value[i].remarks == undefined) {
        this.notification.showError("Please Enter Remarks");
        return false
      }
      if(value[i].display_name=='' || value[i].display_name== null || value[i].display_name == undefined){
        this.notification.showError("Please Enter Display Name");
        return false
      }
      if(value[i].module_id== ''|| value[i].module_id == null || value[i].module_id ==undefined ){
        this.notification.showError("Please select Module");
        return false
      }
    }
