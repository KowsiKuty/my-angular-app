import { Component, OnInit, Output, EventEmitter, ViewChild, Injectable, Input } from '@angular/core';
...
// debounceTime, distinctUntilChanged, tap, filter, switchMap, finalize, 
...
export class BranchEditComponent implements OnInit {
  ...
  @Input() btncenter: string;
  @Input() btnCancelText: string = null;
  dialogSize: string;
  ...
  stateeditfield:any={label: "State",
  ...
  constructor(private fb: FormBuilder, private atmaService: AtmaService,private SpinnerService: NgxSpinnerService,private errorHandler: ErrorHandlingService,
    private notification: NotificationService, private shareService: ShareService,private toastr: ToastrService,private modalService: NgbModal,
    private datePipe: DatePipe,) { 
      this.futureDays.setDate(this.futureDays.getDate());
      // this.pincodeeditfield = {
      //   label: "Pin Code",
      //   method: "get",
      //   url: this.atmaUrl + "mstserv/pincode_search",
      //   params: "&query=",
      //   searchkey: "query",
      //   displaykey: "no",
      //   wholedata: true,
  ...
  ngOnInit(): void {
    ...
    this.pincodeeditfield={ label: "Pin Code",
      method: "get",
      url: this.atmaUrl + "mstserv/pincode_search",
      params: "&query=",
      searchkey: "query",
  ...
  getBranchEdit() {
    ...
    this.address_id = dataa.address_id.id;
    ...
    this.atmaService.getSingleBranch(this.vendorId, this.branchEditID)
      .subscribe(result => {
        let pincodes= result.address_id.pincode_id
        pincodes.city = result.address_id.city_id
        pincodes.district= result.address_id.district_id
        pincodes.state = result.address_id.state_id
        // this.pincodeeditfield = {
        //   label: "Pin Code",
        //   method: "get",
        //   url: this.atmaUrl + "mstserv/pincode_search",
        ...
        this.desname = result.contact_id.designation_id
        // this.designationeditfield={label:"Designation","method":"get","url":this.atmaUrl+"mstserv/designation_search",params:"","searchkey":"query","displaykey":"name",wholedata : true, defaultvalue: this.desname,  required: true}
        ...
        if(this.pan=="" || this.pan==undefined || this.pan==null){
          this.PANNO=false;
        }
  ...
  branchEditForm_() {
    ...
    // if(this.branchEditForm.value.gstno === "" && this.composite_id!=3){
      //bug id:7019
      if(this.branchEditForm.value.gstno === "" && this.composite_id!=3 && this.composite_id!=4){
      this.notification.showWarning(" GST Number Needed..");
      this.SpinnerService.hide();
      return false
    }
    // if(this.isGSTValid == false && this.branchEditForm.value.gstno != "" && this.composite_id!=3 ){
      // BUG ID:7019
    ...
    // if(this.GstNo != this.pan && this.composite_id!=3){
      // BUG UD:7019
      if(this.GstNo != this.pan && this.composite_id!=3 && this.composite_id!=4){
    ...
    // if(this.branchEditForm.value.gstno != "" && this.pan!="" && this.GstNo != this.pan && (this.composite_id==3 || this.composite_id==4)){
  //   if( this.composite_id==4 || this.composite_id==3){
  //   if(this.branchEditForm.value.gstno != "" && this.pan!="" && this.GstNo != this.pan){
  //   this.notification.showWarning(" GST And PAN Not Match..");
  //       this.SpinnerService.hide();
  //     return false
  //   }
  // }
    ...
    if (this.branchEditForm.value.address.pincode_id.city === "" || this.branchEditForm.controls.address.value.pincode_id.city.id === undefined) {
      this.toastr.error('Please Select Valid city');
      this.SpinnerService.hide();
      return false;
    }
    if (this.branchEditForm.value.address.pincode_id.district === "" || this.branchEditForm.controls.address.value.pincode_id.district.id === undefined) {
      this.toastr.error('Please Select Valid district');
      this.SpinnerService.hide();
      return false;
    }
    if (this.branchEditForm.value.address.pincode_id.state === "" || this.branchEditForm.controls.address.value.pincode_id.state.id === undefined) {
    ...
    if(this.branchEditForm.value.gstno!=''){
      let data={
        ...
        "state_id":this.branchEditForm.controls.address.value.pincode_id.state.id
      }
      let msg:any;
      this.func(data)
    }
      ...
      this.branchEditForm.controls.address.value.city_id = this.branchEditForm.controls.address.value.pincode_id.city.id
      this.branchEditForm.controls.address.value.state_id = this.branchEditForm.controls.address.value.pincode_id.state.id
      this.branchEditForm.controls.address.value.district_id = this.branchEditForm.controls.address.value.pincode_id.district.id
    ...
    var str  = this.branchEditForm.value.address.id = this.address_id
    // var cleanStr3=str.trim();//trim() returns string with outer spaces removed
    // this.branchEditForm.value.address.id = cleanStr3 

    var str  = this.branchEditForm.value.contact.id = this.contact_id
    // var cleanStr3=str.trim();//trim() returns string with outer spaces removed
    // this.branchEditForm.value.contact.id = cleanStr3 
  ...
  validationGstNo(e) {
   ...
   let obj = {
    ...
    this.atmaService.getBracnhGSTNo(obj)
      .subscribe(res => {
        let result = res.validation_status
        this.gst_status = result
        this.GSTname = result.lgnm

        if (result.errorCode) {
          this.notification.showWarning(result.errorMessage)
          this.isGSTValid = false
          this.SpinnerService.hide();
        } else {
          // this.notification.showSuccess(" GST Number Validated...")
          this.message = 'GST Validated  ||   Name : ' +this.GSTname ,
          // this.message = `GST Validated 
          // Name : ${this.GSTname}` 
          this.btncenter = 'OK',
          this.btnOkText = 'OK'
          this.dialogSize = 'lg'
        //     const modalRef = this.modalService.open(ConfirmnotificationComponent, { size: this.dialogSize });
        ...
        //   modalRef.componentInstance.btncenter = true;
        //   modalRef.componentInstance.showAcceptButton = true;
        this.customPopupFn()
...
designationeditfield:any={label:"Designation","method":"get","url":this.atmaUrl+"mstserv/designation_search",params:"","searchkey":"query","displaykey":"name",wholedata : true,
  formcontrolname: 'designation_id'
}

// designationeditdata(editdata){
// this.branchEditForm.controls.contact.patchValue({
...
pincodeeditdata(pin){
  ...
  //   disabled: true,
  // };
  this.branchEditForm.controls.address.patchValue({
    // pincode_id: pin,
    city_id: pin.city,
...
customPopupFn() {
  this.customPopup = "flex";
}

customPopupFnClose() {
  this.customPopup = "none";
}
