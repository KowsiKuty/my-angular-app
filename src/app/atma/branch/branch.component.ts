...
import { WindowInterruptSource } from 'ng2-idle';
...
export class BranchComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private atmaService: AtmaService, private modalService: NgbModal,
    ...
    this.designationfield = { label: "Designation", "method": "get", "url": this.vendorURL + "mstserv/designation_search", params: "", "searchkey": "query", "displaykey": "name", wholedata: true, required: true,
  ...
  ngOnInit(): void {
    ...
    //bug id:9146
    this.suppatchdetails = this.shareService.supplierdetails.value
    this.supplierlength = this.shareService.supplen.value
    if (this.supplierlength == 0 || this.supplierlength == undefined) {

      this.branchForm.patchValue({
        address: {

          line1: this.suppatchdetails.line1,
          pincode_id: this.suppatchdetails.pincode,
          city_id: this.suppatchdetails.city,
          district_id: this.suppatchdetails.district,
          state_id: this.suppatchdetails.state,
          line2: this.suppatchdetails.line2,
          line3: this.suppatchdetails.line3,
        }
      })
      let pincodes= this.suppatchdetails.pincode
        pincodes.city = this.suppatchdetails.city
        pincodes.district= this.suppatchdetails.district
        pincodes.state = this.suppatchdetails.state
      // this.pincodefield = {
      //   label: "Pin Code",
      //   method: "get",
      //   url: this.atmaUrl + "mstserv/pincode_search",
  ...
  branchCreateForm() {
    ...
    //bug id:7019
    if (this.branchForm.value.gstno === "" && (this.composite_id != 3 && this.composite_id != 4)) {
      this.notification.showWarning(" GST Number Needed..");
      this.SpinnerService.hide();
      return false
    }
    // if(this.isGSTValid == false && this.branchForm.value.gstno != "" && this.composite_id!=3){
    //bug id:7019
    ...
    // if(this.isGSTValid == false && this.branchForm.value.gstno != "" && this.composite_id ==3 && this.composite_id ==4){
    //   this.notification.showWarning("Please Enter Valid GST");
    //   this.SpinnerService.hide();
    //   return false
    // } 
    ...
    //   if(this.branchForm.value.gstno != "" && this.pan!="" && this.GstNo != this.pan){
    //   this.notification.showWarning(" GST And PAN Not Match..");
    //       this.SpinnerService.hide();
    //     return false
    //   }
    ...
    if (this.branchForm.value.address.pincode_id.city === "" || this.branchForm.controls.address.value.pincode_id.city.id === undefined) {
      this.toastr.error('Please Select Valid city');
      this.SpinnerService.hide();
      return false;
    }
    if (this.branchForm.value.address.pincode_id.district === "" || this.branchForm.controls.address.value.pincode_id.district.id === undefined) {
      this.toastr.error('Please Select Valid district');
      this.SpinnerService.hide();
      return false;
    }
    if (this.branchForm.value.address.pincode_id.state === "" || this.branchForm.controls.address.value.pincode_id.state.id === undefined) {
    ...
    this.branchForm.controls.address.value.city_id = this.branchForm.controls.address.value.pincode_id.city.id
    this.branchForm.controls.address.value.state_id = this.branchForm.controls.address.value.pincode_id.state.id
    this.branchForm.controls.address.value.district_id = this.branchForm.controls.address.value.pincode_id.district.id
    ...
    if (gstno != '') {
      ...
      this.atmaService.getBracnhGSTNo(obj)
        .subscribe(res => {
          ...
          // let BoldGST=GSTname.editor.toggleBold();
          // let bold : boolean = GSTname.selection.characterFormat.bold;      
          // GSTname.selection.characterFormat.bold = true;
  ...
  // designationdata(e) {
  //   console.log("event", e)
  //   this.branchForm.controls.contact.patchValue({
  //     designation_id: e
  //   })
  // }
  ...
  pincodedata(data) {
    ...
    //   disabled: true,
    // };
    this.branchForm.controls.address.patchValue({
      // pincode_id: data,
      city_id: data.city,
