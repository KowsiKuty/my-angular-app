...
export class DocumentQuestionerMappingComponent implements OnInit {
  ...
  quesdoc = [{ 'value': false, 'display': 'Questioner' }];
  venact = [ { 'value': true, 'display': 'Activity' }];
  ...
  getmappinglist(datas){
      ...
      let description_id:FormControl = new FormControl('');
  ...
  mappingdetails(){
    let group = new FormGroup({
      ...
      is_activity:new FormControl(true),
      description_id:new FormControl(''),
  ...
  submitform(){
    ...
    for(let i in DocumentJson){
      ...
      DocumentJson[i].criticality=1
      DocumentJson[i].vendor_type=1
      DocumentJson[i].rel_cat=1
      DocumentJson[i].dept_id=1
      DocumentJson[i].is_activity= true
      
      DocumentJson[i].description_id=DocumentJson[i].description_id.id
