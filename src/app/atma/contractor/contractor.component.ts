...
export class ContractorComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.contractorForm = this.fb.group({
      // panno: ['', [Validators.pattern('[A-Z]{5}[0-9]{4}[A-Z]{1}')]],"^[a-zA-Z]+(\s+[a-zA-Z]+)*$"
      // website: ['', [Validators.pattern("^[a-zA-Z]+(\s+[a-zA-Z]+)*$")]],
  ...
  //   var cleanStr=str.trim();//trim()
  //   this.contractorForm.value.name = cleanStr
  //   console.log("abc",this.contractorForm.value.name)

  // }


  //  whitespaceValidator(form: FormControl): Validators {
