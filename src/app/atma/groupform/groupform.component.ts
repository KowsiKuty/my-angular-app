...
export class GroupformComponent implements OnInit {
  ...
  constructor(private atmaservice: AtmaService, private fb: FormBuilder, private shareService: ShareService, private notification: NotificationService,private spinner:NgxSpinnerService) { }
  ...
  groupsubmit() {
    ...
    this.spinner.show()
    this.atmaservice.creategroup(this.groupform.value).subscribe(
      result => {
        this.spinner.hide()
  ...
  groupmappingsubmit() {
    ...
    this.atmaservice.creategroupmapping(this.groupmappingform.value).subscribe(
      result => {
        if (result.message == 'Successfully Created') {
          this.notification.showSuccess('Successfully Created')
          // this.onCancelClick()
  ...
  getsupplierdata(value) {
    ...
    this.atmaservice.getsupplierdata(this.vendorid, value).subscribe(
      result => {
  ...
  getgroupsummary(page) {
    this.spinner.show()
    this.atmaservice.groupsummary(this.vendorid, page).subscribe(
      result => {
        this.spinner.hide()
  ...
  getmapgroupsummary(id, page) {
    this.atmaservice.groupmapsummary(id, page).subscribe(
      result => {
  ...
  getemployee(value) {
    ...
    this.atmaservice.getportaluseremployee(this.vendorid, value).subscribe(
      result => {
  ...
  getparticulargroupmapping(id) {
    this.atmaservice.getparticulargroupmapping(id).subscribe(
      result => {
        this.groupmappingform.patchValue({
  ...
  getparticulargroupedit(id) {
    ...
    this.atmaservice.getparticulargroup(id).subscribe(
      result => {
        this.groupform.patchValue({
          name: result?.name,
  ...
  grouprulesubmit() {
    ...
    this.atmaservice.createrule(this.ruleform.value).subscribe(
      result => {
  ...
  getrulesummary(page) {
    this.spinner.show()
    this.atmaservice.rulesummary(this.vendorid, page).subscribe(
      result => {
        this.spinner.hide()
  ...
  getparticularruleedit(id) {
    this.spinner.show()
    this.atmaservice.getparticularrule(id).subscribe(
      result => {
  ...
  grouprule() {
    this.atmaservice.getrule().subscribe(
      result => {
  ...
  getproduct(groupid, value) {
    this.atmaservice.getproduct(groupid, value).subscribe(
      result => {
  ...
  deletegroupmap(i, id) {
    this.atmaservice.getparticulargroupdelete(id).subscribe(
      result => {
  ...
  getdynamicrulevalues(id, value) {
    this.atmaservice.getdynamicrulevalues(id, value).subscribe(
      result => {
  ...
  getvendorgroupdropdown(value, page) {
    this.atmaservice.getvendorgroupdropdownsearch(value, page).subscribe(
      result => {
  ...
  editgroup(value) {
    ...
    this.atmaservice.getparticulargroupmapdata(value.id).subscribe(
      result => {
        this.spinner.hide()
        const controlgroup = <FormArray>this.groupform.controls['groupmapping'];
        for (let i = controlgroup.length - 1; i >= 0; i--) {
  ...
  getvendorbasedgroupdropdown(value, page) {
    this.atmaservice.getvendorgroupdropdownsearch(value, page).subscribe(
      result => {
  ...
  getvendorbasedgroup(id, value) {
    this.atmaservice.getvendorbasedgroup(id, value).subscribe(
      result => {
  ...
  getvendorlimit() {
    this.atmaservice.getvendorlimit(this.vendorid).subscribe(
      result => {
        ...
        let data=this.groupform.value.groupmapping
  ...
  calculatelimitofgroup() {
    ...
    const data = this.groupform.value.groupmapping
    // let totallimit = 0;
    let totallimit:any =data.map(_ => _.limit).reduce((x,y) => (+x)+(+y), 0)

    // for (let i = 0; i < data.length; i++) {
    //   totallimit = data[i].limit == '' ? 0 : data[i].limit + totallimit
    //   // console.log('totalimit',totallimit, data[i].limit,  )
    // }
    ...
    if (this.limit.limit == this.limit.grouplimit?.limit__sum) {
      ...
      this.groupform.patchValue({
        limitboolean:totallimit > this.groupform.value.currentlimit
      })
      ...
      this.groupform.patchValue({
        limitboolean:totallimit > this.groupform.value.currentlimit + this.limit.remaininglimits
      })
    }
  ...
  deleteparticularemployeeingroupmap(data, i) {
    ...
    if (data.value.id) {
      ...
      this.atmaservice.getparticularemployeedeleteingroupmapping(data.value.id).subscribe(
        result => {
          if (result.message == "Successfully Deleted") {
