...
@Component({
  selector: 'app-jw-creation',
  templateUrl: './jw-creation.component.html',
  styleUrls: ['./jw-creation.component.scss']
})
export class JwCreationComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder,private datePipe: DatePipe,private jvservice:JwService,
    ...
    private spinnerservice:NgxSpinnerService,public jwshared:ShareService) { }
  ...
  ngOnInit(): void {
    ...
    this.jvstatus = this.jwshared.jvstatus.value
    this.jvid = this.jwshared.jvlist.value
  ...
  submitjv(){
    ...
    if(this.JVcreationForm.value.jerefno != "" && this.JVcreationForm.value.jerefno != undefined && this.JVcreationForm.value.jerefno != null)
  ...
  enableControls() {
    const controlArray = <FormArray>this.JVcreationForm.get('jvcreator');
    controlArray.controls.forEach(group => {
      group.get('jedtype')?.enable();
      // group.get('jeddescription')?.disable();
      group.get('jedamount')?.enable();
      group.get('jedcat_code')?.enable();
      group.get('jedsubcat_code')?.enable();
      group.get('jedglno')?.enable();
      group.get('jedcc_code')?.enable();
      group.get('jedbs_code')?.enable();
      group.get('jedbranch')?.enable();
    });
  }
  ...
  gotoBottom() {
   ...
   ApCategoryClear(index){
      this.JVcreationForm.get('jvcreator')['controls'][index].get('jedcat_code').setValue('')
      this.JVcreationForm.get('jvcreator')['controls'][index].get('jedsubcat_code').setValue('')
  }
  SubApCategoryClear(index){
      this.JVcreationForm.get('jvcreator')['controls'][index].get('jedsubcat_code').setValue('')
  }
  BSClear(index){
      this.JVcreationForm.get('jvcreator')['controls'][index].get('jedbs_code').setValue('')
      this.JVcreationForm.get('jvcreator')['controls'][index].get('jedcc_code').setValue('')
  }
  CClear(index){
      this.JVcreationForm.get('jvcreator')['controls'][index].get('jedcc_code').setValue('')
  }
...
getjvview() {
  this.spinnerservice.show();
  this.jvservice.jwsingleget(this.jvid).subscribe({
    next: (result) => {
      this.spinnerservice.hide();
