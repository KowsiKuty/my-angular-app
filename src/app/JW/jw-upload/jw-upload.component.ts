...
@Component({
  selector: 'app-jw-upload',
  templateUrl: './jw-upload.component.html',
  styleUrls: ['./jw-upload.component.scss']
})
export class JwUploadComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder,private datePipe: DatePipe,private jvservice:JwService,
  ...
  getuploaddata(){
    ...
    this.jvservice.jvbulkupload(this.uplimage).subscribe(result=>{
      ...
      if(this.uploadeddata?.length == 0){
        this.notification.showInfo("No Records Found.Please Check the Excel File")
        return false
      }
      ...
      this.creditsum = (creditamount?.reduce((a, b) => a + b, 0)).toFixed(2)
      let debit = this.uploadeddata?.filter(x=>x.EntryType_id == "Debit")
      let debitamount = debit?.map(c=>c.Amount)
      this.debitsum = (debitamount?.reduce((a, b) => a + b, 0)).toFixed(2)
  ...
  searchcrno(){
    ...
    this.spinnerservice.show()
    let input :any={"type" :'', "crno" :""}
    if(jvrecords.jetype == 2)
      input.type = "rectification_entry"
    else
      input.type = "ta_entry"

   input.crno = jvrecords?.jerefno
   this.spinnerservice.show()
    this.jvservice.crnosearchnew(input)
     .subscribe(res =>{
  ...
  searchjvno(){
    ...
    let jvrecords = this.JVcreationForm?.value
    if(jvrecords?.jerefno == "" || jvrecords?.jerefno == null || jvrecords?.jerefno == undefined){
      this.notification.showError("Please Enter JW Number")
      return false
    }
   
    
     this.spinnerservice.show()
     this.jvservice.jWnosearch(jvrecords?.jerefno)
     .subscribe(res =>{
      
      if(res.code == "Invalid JWCRNO"){
        this.notification.showError(res.description)
        this.spinnerservice.hide()
        return false;
      }
      else if(res.status == "success"){
        this.notification.showSuccess("JW Number Found")
        this.spinnerservice.hide()
      }else{
        this.notification.showError(res.description)
        this.spinnerservice.hide()
  ...
  submitjv(){
    ...
    if(jvheaderdata?.filekeys?.length == 0){
      this.notification.showError("Please Choose File To Bulk Upload")
      return false
    }
    ...
    for(let ind in this.uploadeddata){
      let indd = Number(ind)+1

      if(this.uploadeddata[ind]?.EntryType_id != "Debit" && this.uploadeddata[ind]?.EntryType_id != "Credit"){
        this.notification.showError("Please Enter Valid Entry Type")
        return false
      }
      
      if(this.uploadeddata[ind]?.Branch?.code == "UNEXPECTED_ERROR"){
        this.notification.showError("Please Enter Valid Branch Code")
        return false
      }
      if(this.uploadeddata[ind]?.Category?.code == "UNEXPECTED_ERROR"){
        this.notification.showError("Please Enter Valid AP Category")
        return false
      }
      if(this.uploadeddata[ind]?.Subcategory?.code == "UNEXPECTED_ERROR"){
        this.notification.showError("Please Enter Valid AP Subcategory")
        return false
      }
      if(this.uploadeddata[ind]?.BS?.code == "UNEXPECTED_ERROR"){
        this.notification.showError("Please Enter Valid BS")
        return false
      }
      if(this.uploadeddata[ind]?.CC?.code == "UNEXPECTED_ERROR"){
        this.notification.showError("Please Enter Valid CC")
        return false
      }
      if(this.uploadeddata[ind]?.Amount <= 0){
  ...
  //  console.log("jvdata",jvdata)
    ...
    .subscribe(result =>{
      ...
      this.notification.showSuccess("JW Created Successfully")
      this.spinnerservice.hide()
      this.onSubmit.emit()
      }else{
        this.notification.showError(result.description)
